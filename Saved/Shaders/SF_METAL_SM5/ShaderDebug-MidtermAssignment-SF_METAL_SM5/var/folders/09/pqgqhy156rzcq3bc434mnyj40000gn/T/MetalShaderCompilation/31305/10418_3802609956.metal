...
fragment MainPS_out Main_000028b2_e2a73924(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    ...
    uint _288 = in.in_var_LIGHTMAP_ID * 15u;
    float3 _298 = (_284.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288 + 6u)].xyz;
    float _299 = dot(_298, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _318 = (exp2((_299 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_286 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288 + 7u)], float4(_253.yzx, 1.0)));
    float3 _328 = float4(_298 * (_318 / _299), _318).xyz * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _376 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_185, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _376 = _348;
    ...
    float4 _384 = float4(((mix(float3(0.0), _277, float3(View.View_UnlitViewmodeMask)) + ((_328 * _277) * fast::max(float3(1.0), ((((((_252 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_252 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_252 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _377) * 1.0, 0.0);
    float4 _391;
    ...
    float3 _403 = (_253 * 0.5) + float3(0.5);
    float4 _405 = float4(_403.x, _403.y, _403.z, float4(0.0).w);
    _405.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _202)].y;
    ...
    _408.z = _249;
    float4 _409 = _408;
    _409.w = 0.75686275959014892578125;
    float4 _420 = float4(_252.x, _252.y, _252.z, float4(0.0).w);
    _420.w = ((log2((dot(_328, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_395.xyx * _395.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
