...
vertex Main_out Main_0000331a_9c72eab6(Main_in in [[stage_in]], constant type_View& View [[buffer(2)]], constant type_Primitive& Primitive [[buffer(3)]], constant type_LandscapeContinuousLODParameters& LandscapeContinuousLODParameters [[buffer(4)]], constant type_LandscapeParameters& LandscapeParameters [[buffer(5)]], texture_buffer<float> LandscapeContinuousLODParameters_SectionLOD [[texture(0)]], texture_buffer<float> LandscapeContinuousLODParameters_SectionLODBias [[texture(1)]], texture2d<float> LandscapeParameters_HeightmapTexture [[texture(2)]], texture2d<float> LandscapeParameters_XYOffsetmapTexture [[texture(3)]], sampler LandscapeParameters_HeightmapTextureSampler [[sampler(0)]], sampler LandscapeParameters_XYOffsetmapTextureSampler [[sampler(1)]])
{
    ...
    float2 _380 = _347.xy;
    float4 _385 = float4(_379.x, _379.y, _379.z, _379.w);
    float4 _388 = View.View_TranslatedWorldToClip * _385;
    float2 _391 = ((_380 * LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.zw) + _297;
    float2 _395 = (((_380 * LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.zw) + _256) + _261;
    out.out_var_TEXCOORD0 = (_380 + LandscapeParameters.LandscapeParameters_SubsectionSizeVertsLayerUVPan.zw) + _349;
    out.out_var_TEXCOORD1 = float4(_391.x, _391.y, _395.x, _395.y);
    out.out_var_TEXCOORD2 = float4(0.0);
    out.out_var_TEXCOORD6 = _385;
    out.gl_Position = _388;
    return out;
}
