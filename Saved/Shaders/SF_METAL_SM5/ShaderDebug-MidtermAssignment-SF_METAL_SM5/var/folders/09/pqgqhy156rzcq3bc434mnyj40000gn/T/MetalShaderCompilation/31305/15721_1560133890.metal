...
fragment MainPS_out Main_00003d69_5cfdc102(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], float4 gl_FragCoord [[position]])
{
    ...
    uint _524 = in.in_var_LIGHTMAP_ID * 15u;
    float3 _534 = (_520.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _524 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _524 + 6u)].xyz;
    float _535 = dot(_534, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _554 = (exp2((_535 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_522 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _524 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _524 + 7u)], float4(_466.yzx, 1.0)));
    float3 _564 = float4(_534 * (_554 / _535), _554).xyz * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _614 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_216, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _614 = _586;
    ...
    float4 _622 = float4(((mix(float3(0.0), _496 + (_498 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_564 * _496) * fast::max(float3(1.0), ((((((_465 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_465 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_465 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _615) * 1.0, 0.0);
    float4 _629;
    ...
    float3 _641 = (_466 * 0.5) + float3(0.5);
    float4 _643 = float4(_641.x, _641.y, _641.z, float4(0.0).w);
    _643.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _413)].y;
    ...
    _646.z = _462;
    float4 _647 = _646;
    _647.w = 0.75686275959014892578125;
    float4 _659 = float4(_465.x, _465.y, _465.z, float4(0.0).w);
    _659.w = ((log2(((dot(_564, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_500 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_500 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_500 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_633.xyx * _633.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
