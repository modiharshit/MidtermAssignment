...
fragment MainPS_out Main_00003502_3fee9046(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_StaticShadowTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _250 = _243.y;
    float3 _260 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_250)), float3(0.0), float3(1.0));
    float _261 = fast::clamp(mix(0.0, 1.0, _250), 0.0, 1.0);
    float _262 = fast::clamp(_243.x, 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _269)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _321 = (_261 * _307) + _297.x;
        _322 = (_260 * _291.w) + _291.xyz;
    ...
    float3 _382 = float3(_262);
    float2 _391 = in.in_var_TEXCOORD4.xy * float2(1.0, 0.5);
    float4 _395 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, _391);
    float4 _397 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, (_391 + float2(0.0, 0.5)));
    uint _403 = _331 + 4u;
    uint _408 = _331 + 6u;
    float3 _413 = _395.xyz;
    float3 _441 = ((((_413 * _413) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _403)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _408)].xyz) * ((exp2(((_395.w + ((_397.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _403)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _408)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_397 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331 + 7u)], float4(_323.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _491 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_207, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _491 = _463;
    ...
    float4 _499 = float4(((mix(float3(0.0), _371 + (_373 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_441 * _371) * fast::max(_382, ((((((_322 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _262) + ((_322 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _262) + ((_322 * 2.755199909210205078125) + float3(0.69029998779296875))) * _262))) + _492) * 1.0, 0.0);
    float4 _506;
    ...
    float3 _518 = (_323 * 0.5) + float3(0.5);
    float4 _520 = float4(_518.x, _518.y, _518.z, float4(0.0).w);
    _520.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _269)].y;
    ...
    float4 _536 = float4(_322.x, _322.y, _322.z, float4(0.0).w);
    _536.w = ((log2(((dot(_441, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_382, ((((((_375 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _262) + ((_375 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _262) + ((_375 * 2.755199909210205078125) + float3(0.69029998779296875))) * _262).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_510.xyx * _510.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
