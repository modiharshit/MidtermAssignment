...
fragment MainOpacityPS_out Main_00000893_259da6ea(MainOpacityPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Globals& _Globals [[buffer(1)]], float4 gl_FragCoord [[position]])
{
    MainOpacityPS_out out = {};
    float4 _58 = float4(_50, _50, gl_FragCoord.z, 1.0) * (1.0 / gl_FragCoord.w);
    float _83;
    switch (0u)
    {
        default:
    ...
    float _99 = (-2.0) * log(fast::max(1.0 - (fast::clamp(fast::min(fast::max((1000000.0 - _83) * 0.01666666753590106964111328125, 0.0), 1.0), 0.0, 1.0) * 0.5), 9.9999997473787516355514526367188e-06));
