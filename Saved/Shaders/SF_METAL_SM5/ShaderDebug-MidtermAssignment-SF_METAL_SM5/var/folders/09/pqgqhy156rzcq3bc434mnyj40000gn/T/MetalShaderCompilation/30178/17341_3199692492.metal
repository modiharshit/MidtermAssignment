...
fragment MainPS_out Main_000043bd_beb76ecc(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_Material& Material [[buffer(2)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_VolumetricLightmapBrickAmbientVector [[texture(1)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients0 [[texture(2)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients1 [[texture(3)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients2 [[texture(4)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients3 [[texture(5)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients4 [[texture(6)]], texture3d<float> View_VolumetricLightmapBrickSHCoefficients5 [[texture(7)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(8)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(9)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(10)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(11)]], texture2d<float> Material_Texture2D_0 [[texture(12)]], texture2d<float> Material_Texture2D_1 [[texture(13)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    ...
    float3 _399 = fast::clamp((_238 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float4 _488 = View_VolumetricLightmapBrickAmbientVector.sample(View_SharedBilinearClampedSampler, _425, level(0.0));
    float _504 = _488.x;
    float4 _506 = (((View_VolumetricLightmapBrickSHCoefficients0.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _504) * float4(1.732051372528076171875, 1.732051372528076171875, 1.732051372528076171875, 3.87297916412353515625);
    float _507 = _488.y;
    float4 _509 = (((View_VolumetricLightmapBrickSHCoefficients2.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _507) * float4(1.732051372528076171875, 1.732051372528076171875, 1.732051372528076171875, 3.87297916412353515625);
    float _510 = _488.z;
    float4 _512 = (((View_VolumetricLightmapBrickSHCoefficients4.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _510) * float4(1.732051372528076171875, 1.732051372528076171875, 1.732051372528076171875, 3.87297916412353515625);
    float4 _529 = (((View_VolumetricLightmapBrickSHCoefficients1.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _504) * float4(3.87297916412353515625, 4.472138881683349609375, 3.87297916412353515625, 3.87297916412353515625);
    float4 _531 = (((View_VolumetricLightmapBrickSHCoefficients3.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _507) * float4(3.87297916412353515625, 4.472138881683349609375, 3.87297916412353515625, 3.87297916412353515625);
    float4 _533 = (((View_VolumetricLightmapBrickSHCoefficients5.sample(View_SharedBilinearClampedSampler, _425, level(0.0)) * 2.0) - float4(1.0)) * _510) * float4(3.87297916412353515625, 4.472138881683349609375, 3.87297916412353515625, 3.87297916412353515625);
    float4 _566 = _171;
    _566.y = (-0.48860299587249755859375) * _389.y;
    float4 _569 = _566;
    _569.z = 0.48860299587249755859375 * _389.z;
    float4 _572 = _569;
    _572.w = (-0.48860299587249755859375) * _389.x;
    float3 _573 = _389 * _389;
    float4 _576 = _169;
    _576.x = (1.09254801273345947265625 * _389.x) * _389.y;
    float4 _579 = _576;
    _579.y = ((-1.09254801273345947265625) * _389.y) * _389.z;
    float4 _584 = _579;
    _584.z = 0.3153919875621795654296875 * ((3.0 * _573.z) - 1.0);
    float4 _587 = _584;
    _587.w = ((-1.09254801273345947265625) * _389.x) * _389.z;
    float4 _591 = _572;
    ...
    _602.x = (dot(float4(_504, _506.xyz), _594) + dot(float4(_506.w, _529.xyz), _595)) + (_529.w * _596);
    float3 _608 = _602;
    _608.y = (dot(float4(_507, _509.xyz), _594) + dot(float4(_509.w, _531.xyz), _595)) + (_531.w * _596);
    float3 _614 = _608;
    _614.z = (dot(float4(_510, _512.xyz), _594) + dot(float4(_512.w, _533.xyz), _595)) + (_533.w * _596);
    float3 _619 = (fast::max(float3(0.0), _614) * float3(0.3183098733425140380859375)) * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _666 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_238, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _666 = _641;
    ...
    float4 _674 = float4(((mix(float3(0.0), _467 + (_469 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_619 * _467) * fast::max(float3(1.0), ((((((_388 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_388 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_388 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _667) * 1.0, 0.0);
    float4 _681;
    ...
    float3 _693 = (_389 * 0.5) + float3(0.5);
    ...
    float4 _711 = float4(_388.x, _388.y, _388.z, float4(0.0).w);
