...
fragment MainPS_out Main_00002790_7e6fe589(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_Material& Material [[buffer(2)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> Material_Texture2D_0 [[texture(2)]], texture2d<float> Material_Texture2D_1 [[texture(3)]], texture2d<float> Material_Texture2D_2 [[texture(4)]], texture2d<float> Material_Texture2D_3 [[texture(5)]], sampler View_MaterialTextureBilinearWrapedSampler [[sampler(0)]], sampler View_SharedBilinearClampedSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _226 = Material_Texture2D_2.sample(View_MaterialTextureBilinearWrapedSampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 8.0));
    float3 _230 = float3(_208.x);
    float4 _235 = Material_Texture2D_3.sample(View_MaterialTextureBilinearWrapedSampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 50.0));
    float3 _239 = float3(_208.z);
    float3 _253 = fast::clamp(mix(mix(mix(Material.Material_VectorExpressions[3].xyz, Material.Material_VectorExpressions[3].xyz, float3(_208.y)), _226.xyz * float3(0.25), _230), _235.xyz * float3(0.2249999940395355224609375), _239), float3(0.0), float3(1.0));
    float _254 = fast::clamp(mix(mix(float3(0.0), float3(1.0), _230), float3(0.0), _239).x, 0.0, 1.0);
    float _308;
    if (Primitive.Primitive_UseVolumetricLightmapShadowFromStationaryLights > 0.0)
    {
        float3 _276 = fast::clamp((_167 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float3 _320 = ((_253 - (_253 * _254)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _327 = (mix(float3(0.039999999105930328369140625), _253, float3(_254)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _369 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_167, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _369 = _344;
    ...
    float4 _376 = float4((mix(float3(0.0), _335 + (select(_327, float3(0.0), bool3(_330)) * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + _370) * 1.0, 0.0);
    float4 _383;
    ...
    float2 _388 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    ...
    float4 _398 = float4(_396.x, _396.y, _396.z, float4(0.0).w);
    _398.w = Primitive.Primitive_PerObjectGBufferData;
    float4 _399 = float4(0.0);
    _399.x = _254;
    float4 _400 = _399;
    _400.y = 0.5;
    float4 _401 = _400;
    _401.z = (fast::clamp(mix(mix(float3(0.100000001490116119384765625), float3(mix(0.37999999523162841796875, 0.550000011920928955078125, _226.x)), _230), float3(mix(0.550000011920928955078125, 0.75, _235.x)), _239).x, 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    float4 _402 = _401;
    _402.w = 0.50588238239288330078125;
    float4 _405 = float4(_253.x, _253.y, _253.z, float4(0.0).w);
    _405.w = (fract(dot(_388.xyx * _388.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125;
    out.out_var_SV_Target0 = _383 * View.View_PreExposure;
    out.out_var_SV_Target1 = _398;
    out.out_var_SV_Target2 = _402;
    out.out_var_SV_Target3 = _405;
