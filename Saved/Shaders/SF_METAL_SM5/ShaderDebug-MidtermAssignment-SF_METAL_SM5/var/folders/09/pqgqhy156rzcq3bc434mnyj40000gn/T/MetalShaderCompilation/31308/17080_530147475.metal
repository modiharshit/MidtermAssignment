...
fragment MainPS_out Main_000042b8_1f996893(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], texture2d<float> LightmapResourceCluster_StaticShadowTexture [[texture(5)]], texture2d<float> Material_Texture2D_0 [[texture(6)]], texture2d<float> Material_Texture2D_1 [[texture(7)]], texture2d<float> Material_Texture2D_2 [[texture(8)]], texture2d<float> Material_Texture2D_3 [[texture(9)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float4 _278 = Material_Texture2D_2.sample(Material_Texture2D_2Sampler, _222, bias(View.View_MaterialTextureMipBias));
    float4 _284 = Material_Texture2D_3.sample(Material_Texture2D_3Sampler, _222, bias(View.View_MaterialTextureMipBias));
    float _285 = _284.w;
    float3 _292 = fast::clamp(_278.xyz * mix(0.800000011920928955078125, 1.2000000476837158203125, _273), float3(0.0), float3(1.0));
    float _293 = fast::clamp(fast::min(fast::max(_285, 0.0), 1.0) * 2.0, 0.0, 1.0);
    float _294 = fast::clamp(_285, 0.0, 1.0);
    float _299 = (fast::clamp(mix(0.20000000298023223876953125, mix(0.60000002384185791015625, 1.0, _273), _284.x), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _319 = ((_211.xy / float2(_211.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _350 = (_293 * _339) + _329.y;
        _351 = _329.x;
        _352 = (_292 * _323.w) + _323.xyz;
        _353 = normalize((_247 * _326.w) + ((_326.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _349 = _299;
    ...
    float4 _360 = LightmapResourceCluster_StaticShadowTexture.sample(LightmapResourceCluster_LightMapSampler, in.in_var_TEXCOORD4.zw);
    uint _361 = in.in_var_LIGHTMAP_ID * 15u;
    uint _362 = _361 + 1u;
    float4 _369 = fast::clamp((_360 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _362)]) + ((View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _362)] * (-0.5)) + float4(0.5)), float4(0.0), float4(1.0));
    float3 _386 = ((_352 - (_352 * _351)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _393 = (mix(float3(0.07999999821186065673828125 * _350), _352, float3(_351)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _495 = mix(fast::clamp(dot(_486, _353), 0.0, 1.0), 1.0, _489);
        _496 = _484;
        _497 = mix(_486, _353, float3(_489));
    }
    else
    {
        _495 = 1.0;
    ...
    {
        float3 _593;
        if (any(abs(_221 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _300 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _300 + 19u)].xyz + float3(1.0))))
        {
            _593 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_221, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _593 = _565;
    ...
    float4 _601 = float4(((mix(float3(0.0), _401 + (_403 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_543 * _401) * fast::max(_412, ((((((_352 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _294) + ((_352 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _294) + ((_352 * 2.755199909210205078125) + float3(0.69029998779296875))) * _294))) + _594) * 1.0, 0.0);
    float4 _608;
    ...
    float3 _620 = (_353 * 0.5) + float3(0.5);
    float4 _622 = float4(_620.x, _620.y, _620.z, float4(0.0).w);
    _622.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _301)].y;
    ...
    float4 _638 = float4(_352.x, _352.y, _352.z, float4(0.0).w);
    _638.w = ((log2(((dot(_543, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_412, ((((((_405 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _294) + ((_405 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _294) + ((_405 * 2.755199909210205078125) + float3(0.69029998779296875))) * _294).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_612.xyx * _612.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    ...
    out.out_var_SV_Target5 = (View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _361)] * _369) * _369;
    return out;
}
