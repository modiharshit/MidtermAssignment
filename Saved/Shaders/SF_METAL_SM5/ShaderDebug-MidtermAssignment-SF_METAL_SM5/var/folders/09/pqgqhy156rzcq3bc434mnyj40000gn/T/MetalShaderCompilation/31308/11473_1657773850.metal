...
fragment MainPS_out Main_00002cd1_62cf9f1a(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _416 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_187, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _416 = _388;
    ...
    float4 _424 = float4(((mix(float3(0.0), _298 + (_300 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_366 * _298) * fast::max(float3(1.0), ((((((_267 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_267 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_267 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _417) * 1.0, 0.0);
    float4 _431;
    ...
    float3 _443 = (_268 * 0.5) + float3(0.5);
    float4 _445 = float4(_443.x, _443.y, _443.z, float4(0.0).w);
    _445.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _215)].y;
    ...
    _448.z = _264;
    float4 _449 = _448;
    _449.w = 0.75686275959014892578125;
    float4 _461 = float4(_267.x, _267.y, _267.z, float4(0.0).w);
