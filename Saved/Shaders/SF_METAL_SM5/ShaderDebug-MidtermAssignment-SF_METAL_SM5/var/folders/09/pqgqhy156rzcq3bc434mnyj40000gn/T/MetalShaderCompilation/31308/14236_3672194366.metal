...
fragment MainPS_out Main_0000379c_dae13d3e(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _251 = _244.y;
    float3 _266 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_251)) * _244.x, float3(0.0), float3(1.0));
    float _267 = fast::clamp(mix(Material.Material_ScalarExpressions[0].y, Material.Material_ScalarExpressions[0].z, _251), 0.0, 1.0);
    float _272 = (fast::clamp(mix(Material.Material_ScalarExpressions[0].w, Material.Material_ScalarExpressions[1].x, _251), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _292 = ((_198.xy / float2(_198.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        float _312 = _302.w;
        _324 = (_272 * _312) + _302.z;
        _325 = (0.5 * _312) + _302.y;
        _326 = (_267 * _312) + _302.x;
        _327 = (_266 * _296.w) + _296.xyz;
        _328 = normalize((_233 * _299.w) + ((_299.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _324 = _272;
    ...
    float4 _427 = float4(_328, 1.0);
    float3 _431 = _143;
    _431.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 0u)], _427);
    float3 _435 = _431;
    _435.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 1u)], _427);
    float3 _439 = _435;
    _439.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 2u)], _427);
    float4 _442 = _427.xyzz * _427.yzzx;
    float3 _446 = _143;
    _446.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 3u)], _442);
    float3 _450 = _446;
    _450.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 4u)], _442);
    float3 _454 = _450;
    _454.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 5u)], _442);
    float3 _468 = (float4(_396 * (_416 / _397), _416).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_439 + _454) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_328.x * _328.x) - (_328.y * _328.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    {
        float3 _518;
        if (any(abs(_208 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _273 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _273 + 19u)].xyz + float3(1.0))))
        {
            _518 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_208, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _518 = _490;
    ...
    float4 _526 = float4(((mix(float3(0.0), _358 + (_360 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_468 * _358) * fast::max(float3(1.0), ((((((_327 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_327 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_327 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _519) * 1.0, 0.0);
    float4 _533;
    ...
    float3 _545 = (_328 * 0.5) + float3(0.5);
    float4 _547 = float4(_545.x, _545.y, _545.z, float4(0.0).w);
    _547.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _274)].y;
    ...
    _550.z = _324;
    float4 _551 = _550;
    _551.w = 0.75686275959014892578125;
    float4 _563 = float4(_327.x, _327.y, _327.z, float4(0.0).w);
    _563.w = ((log2(((dot(_468, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_362 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_362 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_362 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_537.xyx * _537.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
