...
fragment MainPS_out Main_000015f5_eb6e34cc(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_TranslucentBasePass& TranslucentBasePass [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<float> TranslucentBasePass_Shared_Fog_IntegratedLightScattering [[texture(0)]], texture2d<float> Material_Texture2D_0 [[texture(1)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler Material_Texture2D_0Sampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _114 = Material_Texture2D_0.sample(Material_Texture2D_0Sampler, float2(in_var_TEXCOORD3[0].x, in_var_TEXCOORD3[0].y), bias(View.View_MaterialTextureMipBias));
    float _128 = fast::clamp(in.in_var_TEXCOORD2.w * _114.x, 0.0, 1.0);
    ...
    float3 _174 = fast::max(mix(in.in_var_TEXCOORD2.xyz * _114.xyz, Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float _201;
    float3 _202;
