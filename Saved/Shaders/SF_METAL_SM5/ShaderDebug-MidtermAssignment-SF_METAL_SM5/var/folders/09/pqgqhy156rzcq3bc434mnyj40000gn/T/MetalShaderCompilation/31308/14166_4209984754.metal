...
fragment MainPS_out Main_00003756_faef44f2(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _251 = _244.y;
    float3 _262 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_251)) * _244.x, float3(0.0), float3(1.0));
    float _263 = fast::clamp(mix(0.0, 1.0, _251), 0.0, 1.0);
    ...
    float2 _373 = in.in_var_TEXCOORD4.xy * float2(1.0, 0.5);
    float4 _378 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, _373);
    float4 _380 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, (_373 + float2(0.0, 0.5)));
    uint _382 = in.in_var_LIGHTMAP_ID * 15u;
    float3 _392 = (_378.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _382 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _382 + 6u)].xyz;
    float _393 = dot(_392, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _412 = (exp2((_393 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_380 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _382 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _382 + 7u)], float4(_324.yzx, 1.0)));
    float4 _423 = float4(_324, 1.0);
    float3 _427 = _143;
    _427.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 0u)], _423);
    float3 _431 = _427;
    _431.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 1u)], _423);
    float3 _435 = _431;
    _435.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 2u)], _423);
    float4 _438 = _423.xyzz * _423.yzzx;
    float3 _442 = _143;
    _442.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 3u)], _438);
    float3 _446 = _442;
    _446.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 4u)], _438);
    float3 _450 = _446;
    _450.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 5u)], _438);
    float3 _464 = (float4(_392 * (_412 / _393), _412).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_435 + _450) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_324.x * _324.x) - (_324.y * _324.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    float4 _522 = float4(((mix(float3(0.0), _354 + (_356 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_464 * _354) * fast::max(float3(1.0), ((((((_323 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_323 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_323 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _515) * 1.0, 0.0);
    float4 _529;
    ...
    float3 _541 = (_324 * 0.5) + float3(0.5);
    float4 _543 = float4(_541.x, _541.y, _541.z, float4(0.0).w);
    _543.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _270)].y;
    ...
    _546.z = _320;
    float4 _547 = _546;
    _547.w = 0.75686275959014892578125;
    float4 _559 = float4(_323.x, _323.y, _323.z, float4(0.0).w);
    _559.w = ((log2(((dot(_464, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_358 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_358 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_358 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_533.xyx * _533.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
