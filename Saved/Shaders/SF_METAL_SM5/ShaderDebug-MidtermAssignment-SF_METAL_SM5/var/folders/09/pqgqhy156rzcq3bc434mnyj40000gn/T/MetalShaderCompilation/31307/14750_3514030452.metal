...
fragment MainPS_out Main_0000399e_d173d974(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], texture2d<float> LightmapResourceCluster_StaticShadowTexture [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _226)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        float _264 = _254.w;
        float _270 = 0.5 * _264;
        _275 = (_224 * _264) + _254.z;
        _276 = _270 + _254.y;
        _277 = _270 + _254.x;
        _278 = (_219 * _248.w) + _248.xyz;
        _279 = normalize((_206 * _251.w) + ((_251.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _275 = _224;
    ...
    float4 _286 = LightmapResourceCluster_StaticShadowTexture.sample(LightmapResourceCluster_LightMapSampler, in.in_var_TEXCOORD4.zw);
    uint _287 = in.in_var_LIGHTMAP_ID * 15u;
    uint _288 = _287 + 1u;
    float4 _295 = fast::clamp((_286 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288)]) + ((View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _288)] * (-0.5)) + float4(0.5)), float4(0.0), float4(1.0));
    float3 _312 = ((_278 - (_278 * _277)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _319 = (mix(float3(0.07999999821186065673828125 * _276), _278, float3(_277)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _420 = mix(fast::clamp(dot(_411, _279), 0.0, 1.0), 1.0, _414);
        _421 = _409;
        _422 = mix(_411, _279, float3(_414));
    }
    else
    {
        _420 = 1.0;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _518 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_198, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _518 = _490;
    ...
    float4 _526 = float4(((mix(float3(0.0), _327 + (_329 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_468 * _327) * fast::max(float3(1.0), ((((((_278 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_278 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_278 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _519) * 1.0, 0.0);
    float4 _533;
    ...
    float3 _545 = (_279 * 0.5) + float3(0.5);
    float4 _547 = float4(_545.x, _545.y, _545.z, float4(0.0).w);
    _547.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _226)].y;
    ...
    float4 _563 = float4(_278.x, _278.y, _278.z, float4(0.0).w);
    ...
    out.out_var_SV_Target5 = (View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _287)] * _295) * _295;
    return out;
}
