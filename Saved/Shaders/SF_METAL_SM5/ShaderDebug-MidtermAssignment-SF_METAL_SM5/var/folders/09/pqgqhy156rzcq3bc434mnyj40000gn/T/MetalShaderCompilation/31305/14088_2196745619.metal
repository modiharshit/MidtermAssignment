...
fragment MainPS_out Main_00003708_82efad93(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _217 = (0.64999997615814208984375 * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _237 = ((_188.xy / float2(_188.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _269 = (0.5 * _257) + _247.y;
        _270 = _257 + _247.x;
        _271 = (float3(0.180000007152557373046875) * _241.w) + _241.xyz;
        _272 = normalize((_206 * _244.w) + ((_244.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _268 = _217;
    ...
    float4 _275 = float4(0.0);
    _275.x = 1.0;
    float4 _276 = _275;
    _276.y = 0.0;
    float3 _290 = ((_271 - (_271 * _270)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _297 = (mix(float3(0.07999999821186065673828125 * _269), _271, float3(_270)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _400 = mix(fast::clamp(dot(_391, _272), 0.0, 1.0), 1.0, _394);
        _401 = _389;
        _402 = mix(_391, _272, float3(_394));
    }
    else
    {
        _400 = 1.0;
    ...
    {
        float3 _498;
        if (any(abs(_198 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _218 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _218 + 19u)].xyz + float3(1.0))))
        {
            _498 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_198, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _498 = _470;
    ...
    float3 _525 = (_272 * 0.5) + float3(0.5);
    float4 _527 = float4(_525.x, _525.y, _525.z, float4(0.0).w);
    _527.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _219)].y;
    float4 _528 = float4(0.0);
    _528.x = _270;
    float4 _529 = _528;
    _529.y = _269;
    float4 _530 = _529;
    _530.z = fast::clamp(_268, 0.0, 0.996078431606292724609375);
    float4 _531 = _530;
    _531.w = 0.768627464771270751953125;
    float4 _543 = float4(_271.x, _271.y, _271.z, float4(0.0).w);
