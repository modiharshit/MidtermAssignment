...
fragment MainPS_out Main_00002cb6_e93f722c(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _365 = float4(_335 * (_355 / _336), _355).xyz * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _415 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_187, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _415 = _387;
    ...
    float4 _423 = float4(((mix(float3(0.0), _297 + (_299 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_365 * _297) * fast::max(float3(1.0), ((((((_266 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_266 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_266 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _416) * 1.0, 0.0);
    float4 _430;
    ...
    float2 _434 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    float3 _442 = (_267 * 0.5) + float3(0.5);
    float4 _444 = float4(_442.x, _442.y, _442.z, float4(0.0).w);
    _444.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _215)].y;
    float4 _445 = float4(0.0);
    _445.x = _265;
    float4 _446 = _445;
    _446.y = _264;
    float4 _447 = _446;
    _447.z = _263;
    float4 _448 = _447;
    _448.w = 0.75686275959014892578125;
    float4 _460 = float4(_266.x, _266.y, _266.z, float4(0.0).w);
    _460.w = ((log2(((dot(_365, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_301 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_301 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_301 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_434.xyx * _434.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _430 * View.View_PreExposure;
    out.out_var_SV_Target1 = _444;
    out.out_var_SV_Target2 = _448;
    out.out_var_SV_Target3 = _460;
