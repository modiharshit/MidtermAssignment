...
fragment MainPixelShader_out Main_0000148c_e41be216(MainPixelShader_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_LightmapDensityPass& LightmapDensityPass [[buffer(1)]], constant type_Globals& _Globals [[buffer(2)]], texture2d<float> LightmapDensityPass_GridTexture [[texture(0)]], texture2d<float> Material_Texture2D_1 [[texture(1)]], sampler LightmapDensityPass_GridTextureSampler [[sampler(0)]], sampler Material_Texture2D_1Sampler [[sampler(1)]])
{
    ...
    float2 _83 = in.in_var_TEXCOORD4.xy * float2(1.0, 0.5);
    float3 _85 = dfdx(in.in_var_TEXCOORD6.xyz);
    float3 _86 = dfdy(in.in_var_TEXCOORD6.xyz);
    float2 _99 = _83 * (_Globals.LightMapResolutionScale * 2.0);
    ...
    out.out_var_SV_Target0 = LightmapDensityPass_GridTexture.sample(LightmapDensityPass_GridTextureSampler, (_83 * _Globals.LightMapResolutionScale)) * ((((((((_126 * _Globals.LightMapDensityDisplayOptions.y) + (float4(_129, _129, _129, 1.0) * _Globals.LightMapDensityDisplayOptions.x)) * _Globals.BuiltLightingAndSelectedFlags.x) + _Globals.BuiltLightingAndSelectedFlags.yyyy) * _Globals.LightMapDensityDisplayOptions.z) + (LightmapDensityPass.LightmapDensityPass_VertexMappedColor * _Globals.LightMapDensityDisplayOptions.w)) * (1.0 - _Globals.BuiltLightingAndSelectedFlags.z)) + (LightmapDensityPass.LightmapDensityPass_DensitySelectedColor * _Globals.BuiltLightingAndSelectedFlags.z));
    return out;
}
