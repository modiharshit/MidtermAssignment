...
fragment MainPS_out Main_0000598e_0a4aa475(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    uint _753 = in.in_var_LIGHTMAP_ID * 15u;
    uint _754 = _753 + 4u;
    uint _759 = _753 + 6u;
    float3 _764 = _745.xyz;
    float3 _792 = ((((_764 * _764) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _754)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _759)].xyz) * ((exp2(((_745.w + ((_747.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _754)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _759)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_747 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _753 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _753 + 7u)], float4(_691.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    float3 _814 = fast::max(mix(float3(0.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _843;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _842 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_253, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _842 = _814;
    ...
    float4 _850 = float4(((mix(float3(0.0), _721 + (_723 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_792 * _721) * fast::max(float3(1.0), ((((((_690 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_690 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_690 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _843) * 1.0, 0.0);
    float4 _857;
    ...
    float2 _861 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    float3 _869 = (_691 * 0.5) + float3(0.5);
    float4 _871 = float4(_869.x, _869.y, _869.z, float4(0.0).w);
    _871.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _639)].y;
    float4 _872 = float4(0.0);
    _872.x = _689;
    float4 _873 = _872;
    _873.y = _688;
    float4 _874 = _873;
    _874.z = _687;
    float4 _875 = _874;
    _875.w = 0.75686275959014892578125;
    float4 _887 = float4(_690.x, _690.y, _690.z, float4(0.0).w);
    _887.w = ((log2(((dot(_792, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_725 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_725 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_725 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_861.xyx * _861.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _857 * View.View_PreExposure;
    out.out_var_SV_Target1 = _871;
    out.out_var_SV_Target2 = _875;
    out.out_var_SV_Target3 = _887;
