...
fragment MainPS_out Main_0000667b_7566a95a(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], texture2d<float> Material_Texture2D_2 [[texture(7)]], texture2d<float> Material_Texture2D_3 [[texture(8)]], texture2d<float> Material_Texture2D_4 [[texture(9)]], texture2d<float> Material_Texture2D_5 [[texture(10)]], texture2d<float> Material_Texture2D_6 [[texture(11)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], sampler Material_Texture2D_4Sampler [[sampler(6)]], sampler Material_Texture2D_5Sampler [[sampler(7)]], sampler Material_Texture2D_6Sampler [[sampler(8)]], float4 gl_FragCoord [[position]])
{
    ...
    float4 _251 = float4((((gl_FragCoord.xy - View.View_ViewRectMin.xy) * View.View_ViewSizeAndInvSize.zw) - float2(0.5)) * float2(2.0, -2.0), _195, 1.0) * (1.0 / gl_FragCoord.w);
    ...
    float _834;
    if (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _733)].z > 0.0)
    {
        float3 _802 = fast::clamp((_261 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
        float4 _813 = float4(View_VolumetricLightmapIndirectionTexture.read(uint3(int4(int(_802.x), int(_802.y), int(_802.z), 0).xyz), 0));
        _834 = View_DirectionalLightShadowingBrickTexture.sample(View_SharedBilinearClampedSampler, ((((_813.xyz * (View.View_VolumetricLightmapBrickSize + 1.0)) + (fract(_802 / float3(_813.w)) * View.View_VolumetricLightmapBrickSize)) + float3(0.5)) * View.View_VolumetricLightmapBrickTexelSize), level(0.0)).x;
    }
    else
    {
        _834 = 1.0;
    }
    float3 _848 = ((_784 - (_784 * _783)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _855 = (mix(float3(0.07999999821186065673828125 * _782), _784, float3(_783)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    bool _858 = View.View_RenderingReflectionCaptureMask != 0.0;
    float3 _863;
    if (_858)
    {
        _863 = _848 + (_855 * 0.449999988079071044921875);
    }
    else
    {
        _863 = _848;
    }
    float4 _869 = float4(_785, 1.0);
    float3 _873 = _196;
    _873.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 0u)], _869);
    float3 _877 = _873;
    _877.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 1u)], _869);
    float3 _881 = _877;
    _881.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 2u)], _869);
    float4 _884 = _869.xyzz * _869.yzzx;
    float3 _888 = _196;
    _888.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 3u)], _884);
    float3 _892 = _888;
    _892.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 4u)], _884);
    float3 _896 = _892;
    _896.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 5u)], _884);
    float3 _931 = fast::max(mix(float3(0.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _960;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _959 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_261, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _959 = _931;
    ...
    float4 _967 = float4(((mix(float3(0.0), _863 + (select(_855, float3(0.0), bool3(_858)) * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((((fast::max(float3(0.0), (_881 + _896) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 6u)].xyz * ((_785.x * _785.x) - (_785.y * _785.y)))) * View.View_SkyLightColor.xyz) * 1.0) * _863) * fast::max(float3(_726), ((((((_784 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _726) + ((_784 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _726) + ((_784 * 2.755199909210205078125) + float3(0.69029998779296875))) * _726))) + _960) * 1.0, 0.0);
    float4 _974;
    ...
    float2 _978 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    float3 _986 = (_785 * 0.5) + float3(0.5);
    float4 _988 = float4(_986.x, _986.y, _986.z, float4(0.0).w);
    _988.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _733)].y;
    float4 _989 = float4(0.0);
    _989.x = _783;
    float4 _990 = _989;
    _990.y = _782;
    float4 _991 = _990;
    _991.z = _781;
    float4 _992 = _991;
    _992.w = 0.50588238239288330078125;
    float4 _995 = float4(_784.x, _784.y, _784.z, float4(0.0).w);
    _995.w = (fract(dot(_978.xyx * _978.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125;
    out.out_var_SV_Target0 = _974 * View.View_PreExposure;
    out.out_var_SV_Target1 = _988;
    out.out_var_SV_Target2 = _992;
    out.out_var_SV_Target3 = _995;
    out.out_var_SV_Target4 = float4(0.0);
    out.out_var_SV_Target5 = float4(_834, 1.0, 1.0, 1.0);
    return out;
}
