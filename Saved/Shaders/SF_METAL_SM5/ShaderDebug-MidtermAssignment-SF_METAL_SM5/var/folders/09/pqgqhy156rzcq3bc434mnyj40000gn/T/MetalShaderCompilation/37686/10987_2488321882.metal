...
vertex MainVS_out Main_00002aeb_9450c75a(MainVS_in in [[stage_in]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_TranslucentDepthPass& TranslucentDepthPass [[buffer(3)]], constant type_SpriteVF& SpriteVF [[buffer(4)]], constant type_Globals& _Globals [[buffer(5)]], texture_buffer<float> CutoutGeometry [[texture(0)]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]])
{
    MainVS_out out = {};
    ...
    float4 _398 = float4(_374[0].x, _374[0].y, _374[0].z, float4(0.0).w);
    _398.w = _324;
    spvUnsafeArray<float4, 1> _408 = { float4(_338.x, _338.y, float4(0.0).z, float4(0.0).w) };
