...
fragment MainPS_out Main_0000481c_cec9c767(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture0 [[texture(5)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture1 [[texture(6)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture2 [[texture(7)]], texture2d<float> Material_Texture2D_0 [[texture(8)]], texture2d<float> Material_Texture2D_1 [[texture(9)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler IndirectLightingCache_IndirectLightingCacheTextureSampler0 [[sampler(2)]], sampler Material_Texture2D_0Sampler [[sampler(3)]], sampler Material_Texture2D_1Sampler [[sampler(4)]], float4 gl_FragCoord [[position]])
{
    ...
    float _262 = _258.z;
    float _285 = _258.y;
    float3 _306 = fast::clamp(mix(mix(Material.Material_VectorExpressions[6].xyz, Material.Material_VectorExpressions[7].xyz, float3(_262)), Material.Material_VectorExpressions[9].xyz, float3(_285)) * _258.x, float3(0.0), float3(1.0));
    float _307 = fast::clamp(mix(mix(Material.Material_ScalarExpressions[0].z, Material.Material_ScalarExpressions[0].w, _262), Material.Material_ScalarExpressions[1].x, _285), 0.0, 1.0);
    float _312 = (fast::clamp(mix(mix(Material.Material_ScalarExpressions[1].y, Material.Material_ScalarExpressions[1].z, _262), Material.Material_ScalarExpressions[1].w, _285), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _332 = ((_219.xy / float2(_219.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _366 = (_307 * _352) + _342.x;
        _367 = (_306 * _336.w) + _336.xyz;
        _368 = normalize((_254 * _339.w) + ((_339.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _364 = _312;
    ...
    if (_378)
    {
        float3 _490 = fast::clamp((_229 * IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveScale) + IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveAdd, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMinUV, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMaxUV);
        ...
        float4 _519 = _163;
        _519.y = (-0.48860299587249755859375) * _368.y;
        float4 _522 = _519;
        _522.z = 0.48860299587249755859375 * _368.z;
        float4 _525 = _522;
        _525.w = (-0.48860299587249755859375) * _368.x;
        float4 _526 = _525;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _564 = mix(fast::clamp(dot(_554, _368), 0.0, 1.0), 1.0, _558);
        _565 = _555;
        _566 = mix(_554, _368, float3(_558));
    }
    else
    {
        _564 = 1.0;
    ...
    float3 _634 = fast::max(mix((Material.Material_VectorExpressions[2].xyz * _262) * Material.Material_ScalarExpressions[0].x, Material.Material_VectorExpressions[3].xyz, float3(Material.Material_ScalarExpressions[0].y)), float3(0.0));
    float3 _663;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        float3 _662;
        if (any(abs(_229 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _313 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _313 + 19u)].xyz + float3(1.0))))
        {
            _662 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_229, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _662 = _634;
    ...
    float4 _670 = float4(((mix(float3(0.0), _460 + (_462 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_612 * _460) * fast::max(float3(1.0), ((((((_367 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_367 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_367 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _663) * 1.0, 0.0);
    float4 _677;
    ...
    float3 _689 = (_368 * 0.5) + float3(0.5);
    float4 _691 = float4(_689.x, _689.y, _689.z, float4(0.0).w);
    _691.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _314)].y;
    ...
    float4 _707 = float4(_367.x, _367.y, _367.z, float4(0.0).w);
    _707.w = ((log2(((dot(_612, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_464 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_464 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_464 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_681.xyx * _681.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
