...
fragment MainPS_out Main_00004ab3_b17543a4(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float4 _248 = Material_Texture2D_0.sample(Material_Texture2D_0Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * float2(10.0)), bias(View.View_MaterialTextureMipBias));
    ...
    float4 _280 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 20.0), bias(View.View_MaterialTextureMipBias));
    float _282 = mix(0.4000000059604644775390625, 1.0, _280.x);
    float4 _286 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * float2(5.0)), bias(View.View_MaterialTextureMipBias));
    float _311;
    switch (0u)
    {
        default:
    ...
    float _316 = _286.y;
    float4 _320 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * float2(0.5)), bias(View.View_MaterialTextureMipBias));
    float _322 = _320.y;
    ...
    float _405;
    if ((Primitive.Primitive_UseSingleSampleShadowFromStationaryLights > 0.0) && _399)
    {
    ...
    {
        float3 _420 = fast::clamp((_240 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float3 _466 = ((_390 - (_390 * _389)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _473 = (mix(float3(0.07999999821186065673828125 * _388), _390, float3(_389)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    {
        float4 _521 = _170;
        _521.y = (-0.48860299587249755859375) * _391.y;
        float4 _524 = _521;
        _524.z = 0.48860299587249755859375 * _391.z;
        float4 _527 = _524;
        _527.w = (-0.48860299587249755859375) * _391.x;
        float3 _528 = _391 * _391;
        float4 _531 = _167;
        _531.x = (1.09254801273345947265625 * _391.x) * _391.y;
        float4 _534 = _531;
        _534.y = ((-1.09254801273345947265625) * _391.y) * _391.z;
        float4 _539 = _534;
        _539.z = 0.3153919875621795654296875 * ((3.0 * _528.z) - 1.0);
        float4 _542 = _539;
        _542.w = ((-1.09254801273345947265625) * _391.x) * _391.z;
        float4 _546 = _527;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _597 = mix(fast::clamp(dot(_587, _391), 0.0, 1.0), 1.0, _591);
        _598 = _588;
        _599 = mix(_587, _391, float3(_591));
    }
    else
    {
        _597 = 1.0;
    ...
    float3 _607 = _168;
    _607.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 0u)], _603);
    float3 _611 = _607;
    _611.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 1u)], _603);
    float3 _615 = _611;
    _615.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 2u)], _603);
    float4 _618 = _603.xyzz * _603.yzzx;
    float3 _622 = _168;
    _622.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 3u)], _618);
    float3 _626 = _622;
    _626.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 4u)], _618);
    float3 _630 = _626;
    _630.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 5u)], _618);
    float3 _645 = (_571 * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_615 + _630) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 6u)].xyz * ((_599.x * _599.x) - (_599.y * _599.y)))) * View.View_SkyLightColor.xyz) * (_598 * _597));
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _692 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_240, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _692 = _667;
    ...
    float4 _700 = float4(((mix(float3(0.0), _481 + (_483 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_645 * _481) * fast::max(float3(1.0), ((((((_390 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_390 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_390 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _693) * 1.0, 0.0);
    float4 _707;
    ...
    float2 _711 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    float3 _719 = (_391 * 0.5) + float3(0.5);
    float4 _721 = float4(_719.x, _719.y, _719.z, float4(0.0).w);
    _721.w = Primitive.Primitive_PerObjectGBufferData;
    float4 _722 = float4(0.0);
    _722.x = _389;
    float4 _723 = _722;
    _723.y = _388;
    float4 _724 = _723;
    _724.z = _387;
    float4 _725 = _724;
    _725.w = 0.50588238239288330078125;
    float4 _737 = float4(_390.x, _390.y, _390.z, float4(0.0).w);
    _737.w = ((log2(((dot(_645, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_485 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_485 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_485 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_711.xyx * _711.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _707 * View.View_PreExposure;
    out.out_var_SV_Target1 = _721;
    out.out_var_SV_Target2 = _725;
    out.out_var_SV_Target3 = _737;
