...
fragment MainPS_out Main_000041d4_c64829bd(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _238 = (0.64999997615814208984375 * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _240)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _291 = _278 + _268.x;
        _292 = (float3(0.180000007152557373046875) * _262.w) + _262.xyz;
    ...
    else
    {
        ...
        _290 = 0.5;
        _291 = 1.0;
        _292 = float3(0.180000007152557373046875);
        _293 = _227;
    }
    ...
    {
        float3 _324 = fast::clamp((_219 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float4 _358 = float4(0.0);
    _358.x = 1.0;
    float4 _359 = _358;
    _359.y = 0.0;
    float3 _373 = ((_292 - (_292 * _291)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _380 = (mix(float3(0.07999999821186065673828125 * _290), _292, float3(_291)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    {
        float4 _428 = _156;
        _428.y = (-0.48860299587249755859375) * _293.y;
        float4 _431 = _428;
        _431.z = 0.48860299587249755859375 * _293.z;
        float4 _434 = _431;
        _434.w = (-0.48860299587249755859375) * _293.x;
        float3 _435 = _293 * _293;
        float4 _438 = _152;
        _438.x = (1.09254801273345947265625 * _293.x) * _293.y;
        float4 _441 = _438;
        _441.y = ((-1.09254801273345947265625) * _293.y) * _293.z;
        float4 _446 = _441;
        _446.z = 0.3153919875621795654296875 * ((3.0 * _435.z) - 1.0);
        float4 _449 = _446;
        _449.w = ((-1.09254801273345947265625) * _293.x) * _293.z;
        float4 _453 = _434;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _504 = mix(fast::clamp(dot(_494, _293), 0.0, 1.0), 1.0, _498);
        _505 = _495;
        _506 = mix(_494, _293, float3(_498));
    }
    else
    {
        _504 = 1.0;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _602 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_219, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _602 = _574;
    ...
    float4 _610 = float4(((mix(float3(0.0), _388 + (_390 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_552 * _388) * fast::max(float3(1.0), ((((((_292 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_292 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_292 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _603) * 1.0, 0.0);
    float4 _617;
    ...
    float3 _629 = (_293 * 0.5) + float3(0.5);
    float4 _631 = float4(_629.x, _629.y, _629.z, float4(0.0).w);
    _631.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _240)].y;
    float4 _632 = float4(0.0);
    _632.x = _291;
    float4 _633 = _632;
    _633.y = _290;
    float4 _634 = _633;
    _634.z = fast::clamp(_289, 0.0, 0.996078431606292724609375);
    float4 _635 = _634;
    _635.w = 0.517647087574005126953125;
    float4 _647 = float4(_292.x, _292.y, _292.z, float4(0.0).w);
