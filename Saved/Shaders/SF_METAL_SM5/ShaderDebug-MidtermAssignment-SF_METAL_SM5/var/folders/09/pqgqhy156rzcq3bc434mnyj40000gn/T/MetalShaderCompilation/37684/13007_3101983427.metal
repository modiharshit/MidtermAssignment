...
fragment MainPS_out Main_000032cf_b8e482c3(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _406 = (_392.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _396 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _396 + 6u)].xyz;
    float _407 = dot(_406, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _426 = (exp2((_407 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_394 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _396 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _396 + 7u)], float4(_337.yzx, 1.0)));
    float3 _436 = float4(_406 * (_426 / _407), _426).xyz * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _486 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_204, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _486 = _458;
    ...
    float4 _494 = float4(((mix(float3(0.0), _367 + (_369 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_436 * _367) * fast::max(_378, ((((((_336 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _276) + ((_336 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _276) + ((_336 * 2.755199909210205078125) + float3(0.69029998779296875))) * _276))) + _487) * 1.0, 0.0);
    float4 _501;
    ...
    float3 _513 = (_337 * 0.5) + float3(0.5);
    float4 _515 = float4(_513.x, _513.y, _513.z, float4(0.0).w);
    _515.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _283)].y;
    ...
    _518.z = _333;
    float4 _519 = _518;
    _519.w = 0.75686275959014892578125;
    float4 _531 = float4(_336.x, _336.y, _336.z, float4(0.0).w);
    _531.w = ((log2(((dot(_436, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_378, ((((((_371 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _276) + ((_371 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _276) + ((_371 * 2.755199909210205078125) + float3(0.69029998779296875))) * _276).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_505.xyx * _505.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
