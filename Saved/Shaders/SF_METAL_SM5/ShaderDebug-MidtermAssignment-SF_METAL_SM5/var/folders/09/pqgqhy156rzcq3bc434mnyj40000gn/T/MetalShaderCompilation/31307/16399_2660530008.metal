...
fragment MainPS_out Main_0000400f_9e947758(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture0 [[texture(5)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture1 [[texture(6)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture2 [[texture(7)]], texture2d<float> Material_Texture2D_0 [[texture(8)]], texture2d<float> Material_Texture2D_1 [[texture(9)]], texture2d<float> Material_Texture2D_2 [[texture(10)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler IndirectLightingCache_IndirectLightingCacheTextureSampler0 [[sampler(2)]], sampler Material_Texture2D_0Sampler [[sampler(3)]], sampler Material_Texture2D_1Sampler [[sampler(4)]], sampler Material_Texture2D_2Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _228 = _224.xyz / float3(_224.w);
    float3 _229 = _228 - View.View_PreViewTranslation;
    ...
    float2 _244 = ((normalize(-_228) * _208).xy * ((0.0500000007450580596923828125 * _238.z) + (-0.02500000037252902984619140625))) + float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y);
    ...
    float3 _267 = normalize(_208 * normalize((float4(_251, sqrt(fast::clamp(1.0 - dot(_251, _251), 0.0, 1.0)), 1.0).xyz * View.View_NormalOverrideParameter.w) + View.View_NormalOverrideParameter.xyz)) * 1.0;
    float4 _269 = Material_Texture2D_0.sample(Material_Texture2D_0Sampler, _244, bias(View.View_MaterialTextureMipBias));
    float _282 = _269.y;
    ...
    float3 _302 = fast::clamp(float3(mix(0.25, 0.4000000059604644775390625, _294.x) * 0.25), float3(0.0), float3(1.0));
    float _307 = (fast::clamp(fast::min(fast::max(0.25 + _282, 0.0), 1.0), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _309)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _359 = _337.x;
        _360 = (_302 * _331.w) + _331.xyz;
        _361 = normalize((_267 * _334.w) + ((_334.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _357 = _307;
    ...
    if (_371)
    {
        float3 _483 = fast::clamp((_229 * IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveScale) + IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveAdd, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMinUV, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMaxUV);
        ...
        float4 _512 = _164;
        _512.y = (-0.48860299587249755859375) * _361.y;
        float4 _515 = _512;
        _515.z = 0.48860299587249755859375 * _361.z;
        float4 _518 = _515;
        _518.w = (-0.48860299587249755859375) * _361.x;
        float4 _519 = _518;
    ...
    float3 _556 = fast::max(mix(mix(Material.Material_VectorExpressions[3].xyz, Material.Material_VectorExpressions[4].xyz, float3(sin(View.View_GameTime * 1.57079637050628662109375))) * _282, Material.Material_VectorExpressions[5].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _585;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _584 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_229, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _584 = _556;
    ...
    float4 _592 = float4(((mix(float3(0.0), _453 + (_455 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_534 * _453) * fast::max(float3(1.0), ((((((_360 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_360 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_360 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _585) * 1.0, 0.0);
    float4 _599;
    ...
    float3 _611 = (_361 * 0.5) + float3(0.5);
    float4 _613 = float4(_611.x, _611.y, _611.z, float4(0.0).w);
    _613.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _309)].y;
