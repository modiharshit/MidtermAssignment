...
fragment MainPS_out Main_0000363a_df15a6ab(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    ...
    float _273 = _268.x;
    float3 _285 = fast::clamp(mix(Material.Material_VectorExpressions[3].xyz, _264.xyz, float3(_273)) * mix(0.75, 1.0, _259), float3(0.0), float3(1.0));
    float _286 = fast::clamp(mix(0.0, 1.0, _273 + _268.z), 0.0, 1.0);
    float _287 = fast::clamp(_268.w, 0.0, 1.0);
    float _292 = (fast::clamp(mix(0.300000011920928955078125, 0.100000001490116119384765625, _259 * (0.75 + _268.y)), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _312 = ((_198.xy / float2(_198.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _348 = normalize((_233 * _319.w) + ((_319.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _344 = _292;
    ...
    float3 _417 = (_403.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _407 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _407 + 6u)].xyz;
    float _418 = dot(_417, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _437 = (exp2((_418 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_405 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _407 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _407 + 7u)], float4(_348.yzx, 1.0)));
    float3 _447 = float4(_417 * (_437 / _418), _437).xyz * View.View_IndirectLightingColorScale;
    ...
    {
        float3 _497;
        if (any(abs(_208 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _293 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _293 + 19u)].xyz + float3(1.0))))
        {
            _497 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_208, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _497 = _469;
    ...
    float4 _505 = float4(((mix(float3(0.0), _378 + (_380 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_447 * _378) * fast::max(_389, ((((((_347 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _287) + ((_347 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _287) + ((_347 * 2.755199909210205078125) + float3(0.69029998779296875))) * _287))) + _498) * 1.0, 0.0);
    float4 _512;
    ...
    float3 _524 = (_348 * 0.5) + float3(0.5);
    float4 _526 = float4(_524.x, _524.y, _524.z, float4(0.0).w);
    _526.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _294)].y;
