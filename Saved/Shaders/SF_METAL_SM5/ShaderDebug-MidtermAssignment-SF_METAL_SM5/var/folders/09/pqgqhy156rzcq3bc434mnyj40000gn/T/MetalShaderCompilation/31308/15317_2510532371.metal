...
fragment MainPS_out Main_00003bd5_95a3af13(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _241 = normalize(float3x3(in.in_var_TEXCOORD10_centroid.xyz, cross(in.in_var_TEXCOORD11_centroid.xyz, in.in_var_TEXCOORD10_centroid.xyz) * in.in_var_TEXCOORD11_centroid.w, in.in_var_TEXCOORD11_centroid.xyz) * normalize((float4(_225, sqrt(fast::clamp(1.0 - dot(_225, _225), 0.0, 1.0)), 1.0).xyz * View.View_NormalOverrideParameter.w) + View.View_NormalOverrideParameter.xyz)) * 1.0;
    float4 _253 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 0.21340000629425048828125), bias(View.View_MaterialTextureMipBias));
    float4 _258 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 0.053410001099109649658203125), bias(View.View_MaterialTextureMipBias));
    float4 _263 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, (float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) * 0.00200000009499490261077880859375), bias(View.View_MaterialTextureMipBias));
    float _267 = (_253.x + 0.5) * ((_258.x + 0.5) * (_263.x + 0.5));
    float4 _272 = Material_Texture2D_2.sample(Material_Texture2D_2Sampler, _216, bias(View.View_MaterialTextureMipBias));
    float4 _278 = Material_Texture2D_3.sample(Material_Texture2D_3Sampler, _216, bias(View.View_MaterialTextureMipBias));
    float _279 = _278.w;
    float3 _286 = fast::clamp(_272.xyz * mix(0.800000011920928955078125, 1.2000000476837158203125, _267), float3(0.0), float3(1.0));
    float _287 = fast::clamp(fast::min(fast::max(_279, 0.0), 1.0) * 2.0, 0.0, 1.0);
    float _288 = fast::clamp(_279, 0.0, 1.0);
    float _293 = (fast::clamp(mix(0.20000000298023223876953125, mix(0.60000002384185791015625, 1.0, _267), _278.x), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _295)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _344 = (_287 * _333) + _323.y;
        _345 = _323.x;
        _346 = (_286 * _317.w) + _317.xyz;
        _347 = normalize((_241 * _320.w) + ((_320.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _343 = _293;
    ...
    float _436 = (exp2((_417 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_404 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _406 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _406 + 7u)], float4(_347.yzx, 1.0)));
    float4 _447 = float4(_347, 1.0);
    float3 _451 = _150;
    ...
    float3 _488 = (float4(_416 * (_436 / _417), _436).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_459 + _474) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_347.x * _347.x) - (_347.y * _347.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _538 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_215, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _538 = _510;
    ...
    float4 _546 = float4(((mix(float3(0.0), _377 + (_379 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_488 * _377) * fast::max(_388, ((((((_346 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _288) + ((_346 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _288) + ((_346 * 2.755199909210205078125) + float3(0.69029998779296875))) * _288))) + _539) * 1.0, 0.0);
    float4 _553;
    ...
    float3 _565 = (_347 * 0.5) + float3(0.5);
    float4 _567 = float4(_565.x, _565.y, _565.z, float4(0.0).w);
    _567.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _295)].y;
    ...
    float4 _583 = float4(_346.x, _346.y, _346.z, float4(0.0).w);
    _583.w = ((log2(((dot(_488, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_388, ((((((_381 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _288) + ((_381 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _288) + ((_381 * 2.755199909210205078125) + float3(0.69029998779296875))) * _288).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_557.xyx * _557.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
