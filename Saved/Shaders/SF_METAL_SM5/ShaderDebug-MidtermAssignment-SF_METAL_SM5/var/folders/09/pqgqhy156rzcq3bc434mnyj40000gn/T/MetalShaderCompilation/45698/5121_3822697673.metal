...
fragment MainPixelShader_out Main_00001401_e3d9bcc9(MainPixelShader_in in [[stage_in]], constant type_LightmapDensityPass& LightmapDensityPass [[buffer(0)]], constant type_Globals& _Globals [[buffer(1)]], texture2d<float> LightmapDensityPass_GridTexture [[texture(0)]], texture2d<float> Material_Texture2D_0 [[texture(1)]], sampler LightmapDensityPass_GridTextureSampler [[sampler(0)]], sampler Material_Texture2D_0Sampler [[sampler(1)]])
{
    ...
    float2 _82 = in.in_var_TEXCOORD4.xy * float2(1.0, 0.5);
    float3 _84 = dfdx(in.in_var_TEXCOORD6.xyz);
    float3 _85 = dfdy(in.in_var_TEXCOORD6.xyz);
    float2 _98 = _82 * (_Globals.LightMapResolutionScale * 2.0);
    float2 _99 = dfdx(_98);
    ...
    out.out_var_SV_Target0 = LightmapDensityPass_GridTexture.sample(LightmapDensityPass_GridTextureSampler, (_82 * _Globals.LightMapResolutionScale)) * ((((((((_125 * _Globals.LightMapDensityDisplayOptions.y) + (float4(_128, _128, _128, 1.0) * _Globals.LightMapDensityDisplayOptions.x)) * _Globals.BuiltLightingAndSelectedFlags.x) + _Globals.BuiltLightingAndSelectedFlags.yyyy) * _Globals.LightMapDensityDisplayOptions.z) + (LightmapDensityPass.LightmapDensityPass_VertexMappedColor * _Globals.LightMapDensityDisplayOptions.w)) * (1.0 - _Globals.BuiltLightingAndSelectedFlags.z)) + (LightmapDensityPass.LightmapDensityPass_DensitySelectedColor * _Globals.BuiltLightingAndSelectedFlags.z));
    return out;
}
