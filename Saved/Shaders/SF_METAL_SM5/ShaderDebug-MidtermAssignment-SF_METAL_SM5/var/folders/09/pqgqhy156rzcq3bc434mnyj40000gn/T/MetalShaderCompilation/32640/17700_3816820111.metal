...
fragment MainPS_out Main_00004524_e3800d8f(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], texture2d<float> LightmapResourceCluster_StaticShadowTexture [[texture(5)]], texture2d<float> Material_Texture2D_0 [[texture(6)]], texture2d<float> Material_Texture2D_1 [[texture(7)]], texture2d<float> Material_Texture2D_2 [[texture(8)]], texture2d<float> Material_Texture2D_3 [[texture(9)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    ...
    float3 _324 = fast::clamp(_318.xyz * _314, float3(0.0), float3(1.0));
    float _329 = (fast::clamp(mix(0.800000011920928955078125, 0.4000000059604644775390625, _314 * _318.w), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _349 = ((_215.xy / float2(_215.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _383 = normalize((_288 * _356.w) + ((_356.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _379 = _329;
    ...
    float3 _416 = ((_382 - (_382 * _381)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _423 = (mix(float3(0.07999999821186065673828125 * _380), _382, float3(_381)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    float4 _454 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, _450);
    float4 _456 = LightmapResourceCluster_LightMapTexture.sample(LightmapResourceCluster_LightMapSampler, (_450 + float2(0.0, 0.5)));
    uint _462 = _391 + 4u;
    uint _467 = _391 + 6u;
    float3 _472 = _454.xyz;
    float _524;
    float _525;
    float3 _526;
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        float4 _508 = LightmapResourceCluster_SkyOcclusionTexture.sample(LightmapResourceCluster_LightMapSampler, in.in_var_TEXCOORD4.xy);
        float _512 = _508.w;
        float _513 = _512 * _512;
        float3 _515 = normalize(((_508.xyz * 2.0) - float3(1.0)).xyz);
        float _516 = 1.0 - _513;
        float _518 = 1.0 - (_516 * _516);
        _524 = mix(fast::clamp(dot(_515, _383), 0.0, 1.0), 1.0, _518);
        _525 = _513;
        _526 = mix(_515, _383, float3(_518));
    }
    else
    {
        _524 = 1.0;
    ...
    float4 _530 = float4(_526, 1.0);
    float3 _534 = _159;
    _534.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 0u)], _530);
    float3 _538 = _534;
    _538.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 1u)], _530);
    float3 _542 = _538;
    _542.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 2u)], _530);
    float4 _545 = _530.xyzz * _530.yzzx;
    float3 _549 = _159;
    _549.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 3u)], _545);
    float3 _553 = _549;
    _553.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 4u)], _545);
    float3 _557 = _553;
    _557.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 5u)], _545);
    float3 _572 = (((((_472 * _472) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _462)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _467)].xyz) * ((exp2(((_454.w + ((_456.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _462)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _467)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_456 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _391 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _391 + 7u)], float4(_383.yzx, 1.0))))) * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_542 + _557) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_526.x * _526.x) - (_526.y * _526.y)))) * View.View_SkyLightColor.xyz) * (_525 * _524));
    float3 _594 = fast::max(mix(float3(0.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _623;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        float3 _622;
        if (any(abs(_224 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _330 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _330 + 19u)].xyz + float3(1.0))))
        {
            _622 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_224, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _622 = _594;
    ...
    float4 _630 = float4(((mix(float3(0.0), _431 + (_433 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_572 * _431) * fast::max(float3(1.0), ((((((_382 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_382 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_382 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _623) * 1.0, 0.0);
    float4 _637;
    ...
    float3 _649 = (_383 * 0.5) + float3(0.5);
    float4 _651 = float4(_649.x, _649.y, _649.z, float4(0.0).w);
    _651.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _331)].y;
    ...
    float4 _667 = float4(_382.x, _382.y, _382.z, float4(0.0).w);
    _667.w = ((log2(((dot(_572, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_435 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_435 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_435 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_641.xyx * _641.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
