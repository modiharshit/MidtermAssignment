...
fragment MainPS_out Main_0000153e_fab6e4b3(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_TranslucentBasePass& TranslucentBasePass [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<float> TranslucentBasePass_Shared_Fog_IntegratedLightScattering [[texture(0)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], float4 gl_FragCoord [[position]])
{
    ...
    float2 _112 = float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y) - float2(0.5);
    float _118 = fast::min(fast::max(1.0 - (sqrt(dot(_112, _112)) * 2.0), 0.0), 1.0);
    float _124 = fast::clamp(in.in_var_TEXCOORD5.w * ((_118 <= 0.0) ? 0.0 : pow(_118, 4.0)), 0.0, 1.0);
    ...
    float3 _177 = fast::max(mix(in.in_var_TEXCOORD5.xyz, Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float _204;
    float3 _205;
