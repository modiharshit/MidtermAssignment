...
fragment MainPS_out Main_00004656_c22fb536(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _273 = _266.y;
    float3 _284 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_273)) * _266.x, float3(0.0), float3(1.0));
    float _285 = fast::clamp(mix(0.0, 1.0, _273), 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _292)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _344 = (_285 * _330) + _320.x;
        _345 = (_284 * _314.w) + _314.xyz;
    ...
    {
        float4 _478 = _163;
        _478.y = (-0.48860299587249755859375) * _346.y;
        float4 _481 = _478;
        _481.z = 0.48860299587249755859375 * _346.z;
        float4 _484 = _481;
        _484.w = (-0.48860299587249755859375) * _346.x;
        float3 _485 = _346 * _346;
        float4 _488 = _159;
        _488.x = (1.09254801273345947265625 * _346.x) * _346.y;
        float4 _491 = _488;
        _491.y = ((-1.09254801273345947265625) * _346.y) * _346.z;
        float4 _496 = _491;
        _496.z = 0.3153919875621795654296875 * ((3.0 * _485.z) - 1.0);
        float4 _499 = _496;
        _499.w = ((-1.09254801273345947265625) * _346.x) * _346.z;
        float4 _503 = _484;
        ...
        _514.x = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[0], _506) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[0], _507)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.x * _508);
        float3 _520 = _514;
        _520.y = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[1], _506) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[1], _507)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.y * _508);
        float3 _526 = _520;
        _526.z = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[2], _506) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[2], _507)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.z * _508);
        _528 = fast::max(float3(0.0), _526);
    }
    else
    {
        _528 = float3(0.0);
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        if (_356)
        {
            _544 = IndirectLightingCache.IndirectLightingCache_PointSkyBentNormal.xyz;
            _545 = IndirectLightingCache.IndirectLightingCache_PointSkyBentNormal.w;
        }
        else
        {
            _544 = _346;
        ...
        _554 = mix(fast::clamp(dot(_544, _346), 0.0, 1.0), 1.0, _548);
        _555 = _545;
        _556 = mix(_544, _346, float3(_548));
    }
    else
    {
        _554 = 1.0;
    ...
    float3 _602 = (_528 * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_572 + _587) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 6u)].xyz * ((_556.x * _556.x) - (_556.y * _556.y)))) * View.View_SkyLightColor.xyz) * (_555 * _554));
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _652 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_230, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _652 = _624;
    ...
    float4 _660 = float4(((mix(float3(0.0), _438 + (_440 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_602 * _438) * fast::max(float3(1.0), ((((((_345 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_345 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_345 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _653) * 1.0, 0.0);
    float4 _667;
    ...
    float3 _679 = (_346 * 0.5) + float3(0.5);
    float4 _681 = float4(_679.x, _679.y, _679.z, float4(0.0).w);
    _681.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _292)].y;
    ...
    float4 _697 = float4(_345.x, _345.y, _345.z, float4(0.0).w);
    _697.w = ((log2(((dot(_602, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_442 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_442 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_442 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_671.xyx * _671.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
