...
fragment MainPS_out Main_000032d4_39055e4a(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    ...
    float _215 = (0.64999997615814208984375 * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _235 = ((_186.xy / float2(_186.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        float _255 = _245.w;
        _266 = (_215 * _255) + _245.z;
        _267 = (0.5 * _255) + _245.y;
        _268 = _255 + _245.x;
        _269 = (float3(0.180000007152557373046875) * _239.w) + _239.xyz;
        _270 = normalize((_204 * _242.w) + ((_242.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _266 = _215;
    ...
    float4 _273 = float4(0.0);
    _273.x = 1.0;
    float4 _274 = _273;
    _274.y = 0.0;
    float3 _288 = ((_269 - (_269 * _268)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _295 = (mix(float3(0.07999999821186065673828125 * _267), _269, float3(_268)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    float _361 = (exp2((_342 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_329 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331 + 7u)], float4(_270.yzx, 1.0)));
    float4 _372 = float4(_270, 1.0);
    float3 _376 = _135;
    ...
    float3 _413 = (float4(_341 * (_361 / _342), _361).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_384 + _399) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_270.x * _270.x) - (_270.y * _270.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    {
        float3 _463;
        if (any(abs(_196 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _216 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _216 + 19u)].xyz + float3(1.0))))
        {
            _463 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_196, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _463 = _435;
    ...
    float3 _490 = (_270 * 0.5) + float3(0.5);
    float4 _492 = float4(_490.x, _490.y, _490.z, float4(0.0).w);
    _492.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _217)].y;
    float4 _493 = float4(0.0);
    _493.x = _268;
    float4 _494 = _493;
    _494.y = _267;
    float4 _495 = _494;
    _495.z = fast::clamp(_266, 0.0, 0.996078431606292724609375);
    float4 _496 = _495;
    _496.w = 0.768627464771270751953125;
    float4 _508 = float4(_269.x, _269.y, _269.z, float4(0.0).w);
