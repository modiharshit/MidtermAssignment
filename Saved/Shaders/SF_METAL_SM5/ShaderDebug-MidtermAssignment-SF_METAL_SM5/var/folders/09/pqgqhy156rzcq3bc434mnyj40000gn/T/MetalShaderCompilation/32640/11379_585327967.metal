...
fragment MainPS_out Main_00002c73_22e3655f(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    ...
    uint _316 = in.in_var_LIGHTMAP_ID * 15u;
    uint _317 = _316 + 4u;
    uint _322 = _316 + 6u;
    float3 _327 = _308.xyz;
    float3 _355 = ((((_327 * _327) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _317)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _322)].xyz) * ((exp2(((_308.w + ((_310.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _317)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _322)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_310 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _316 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _316 + 7u)], float4(_254.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    float3 _377 = fast::max(mix(float3(0.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _406;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _405 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_187, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _405 = _377;
    ...
    float4 _413 = float4(((mix(float3(0.0), _284 + (_286 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_355 * _284) * fast::max(float3(1.0), ((((((_253 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_253 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_253 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _406) * 1.0, 0.0);
    float4 _420;
    ...
    float2 _424 = (fract(gl_FragCoord.xy * float2(0.0078125)) * 128.0) + float2(-64.3406219482421875, -72.4656219482421875);
    float3 _432 = (_254 * 0.5) + float3(0.5);
    float4 _434 = float4(_432.x, _432.y, _432.z, float4(0.0).w);
    _434.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _205)].y;
    float4 _435 = float4(0.0);
    _435.x = _252;
    float4 _436 = _435;
    _436.y = _251;
    float4 _437 = _436;
    _437.z = _250;
    float4 _438 = _437;
    _438.w = 0.75686275959014892578125;
    float4 _450 = float4(_253.x, _253.y, _253.z, float4(0.0).w);
    _450.w = ((log2(((dot(_355, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_288 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_288 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_288 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_424.xyx * _424.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _420 * View.View_PreExposure;
    out.out_var_SV_Target1 = _434;
    out.out_var_SV_Target2 = _438;
