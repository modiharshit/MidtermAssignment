...
fragment MainPS_out Main_00003d2c_bc46879d(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _267 = _260.y;
    float3 _277 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_267)), float3(0.0), float3(1.0));
    float _278 = fast::clamp(mix(0.0, 1.0, _267), 0.0, 1.0);
    float _279 = fast::clamp(_260.x, 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _286)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _338 = (_278 * _324) + _314.x;
        _339 = (_277 * _308.w) + _308.xyz;
    ...
    float3 _443 = float3(_279);
    float3 _523;
    if (_350)
    {
        float4 _473 = _161;
        _473.y = (-0.48860299587249755859375) * _340.y;
        float4 _476 = _473;
        _476.z = 0.48860299587249755859375 * _340.z;
        float4 _479 = _476;
        _479.w = (-0.48860299587249755859375) * _340.x;
        float3 _480 = _340 * _340;
        float4 _483 = _158;
        _483.x = (1.09254801273345947265625 * _340.x) * _340.y;
        float4 _486 = _483;
        _486.y = ((-1.09254801273345947265625) * _340.y) * _340.z;
        float4 _491 = _486;
        _491.z = 0.3153919875621795654296875 * ((3.0 * _480.z) - 1.0);
        float4 _494 = _491;
        _494.w = ((-1.09254801273345947265625) * _340.x) * _340.z;
        float4 _498 = _479;
        ...
        _509.x = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[0], _501) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[0], _502)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.x * _503);
        float3 _515 = _509;
        _515.y = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[1], _501) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[1], _502)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.y * _503);
        float3 _521 = _515;
        _521.z = (dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients0[2], _501) + dot(IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients1[2], _502)) + (IndirectLightingCache.IndirectLightingCache_IndirectLightingSHCoefficients2.z * _503);
        _523 = fast::max(float3(0.0), _521);
    }
    else
    {
        _523 = float3(0.0);
    }
    float3 _526 = _523 * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _576 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_224, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _576 = _548;
    ...
    float4 _584 = float4(((mix(float3(0.0), _432 + (_434 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_526 * _432) * fast::max(_443, ((((((_339 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _279) + ((_339 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _279) + ((_339 * 2.755199909210205078125) + float3(0.69029998779296875))) * _279))) + _577) * 1.0, 0.0);
    float4 _591;
    ...
    float3 _603 = (_340 * 0.5) + float3(0.5);
    float4 _605 = float4(_603.x, _603.y, _603.z, float4(0.0).w);
    _605.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _286)].y;
    ...
    _608.z = _336;
    float4 _609 = _608;
    _609.w = 0.50588238239288330078125;
    float4 _621 = float4(_339.x, _339.y, _339.z, float4(0.0).w);
    _621.w = ((log2(((dot(_526, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_443, ((((((_436 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _279) + ((_436 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _279) + ((_436 * 2.755199909210205078125) + float3(0.69029998779296875))) * _279).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_595.xyx * _595.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
