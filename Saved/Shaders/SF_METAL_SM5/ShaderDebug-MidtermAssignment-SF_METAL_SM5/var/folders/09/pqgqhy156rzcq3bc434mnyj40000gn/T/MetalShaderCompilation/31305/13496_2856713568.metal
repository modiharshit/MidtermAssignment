...
fragment MainPS_out Main_000034b8_aa45fd60(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], float4 gl_FragCoord [[position]])
{
    ...
    float _274 = (fast::clamp(mix(0.60000002384185791015625, 0.4000000059604644775390625, _257 * _263.w), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _276)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        float _314 = _304.w;
        _325 = (_274 * _314) + _304.z;
        _326 = (0.5 * _314) + _304.y;
        _327 = _314 + _304.x;
        _328 = (_269 * _298.w) + _298.xyz;
        _329 = normalize((_231 * _301.w) + ((_301.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _325 = _274;
    ...
    uint _392 = _391 + 4u;
    uint _397 = _391 + 6u;
    float3 _402 = _383.xyz;
    float3 _430 = ((((_402 * _402) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _392)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _397)].xyz) * ((exp2(((_383.w + ((_385.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _392)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _397)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_385 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _391 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _391 + 7u)], float4(_329.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _480 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_206, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _480 = _452;
    ...
    float4 _488 = float4(((mix(float3(0.0), _359 + (_361 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_430 * _359) * fast::max(float3(1.0), ((((((_328 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_328 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_328 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _481) * 1.0, 0.0);
    float4 _495;
    ...
    float3 _507 = (_329 * 0.5) + float3(0.5);
    float4 _509 = float4(_507.x, _507.y, _507.z, float4(0.0).w);
    _509.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _276)].y;
    ...
    _512.z = _325;
    float4 _513 = _512;
    _513.w = 0.75686275959014892578125;
    float4 _525 = float4(_328.x, _328.y, _328.z, float4(0.0).w);
    _525.w = ((log2(((dot(_430, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_363 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_363 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_363 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_499.xyx * _499.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _495 * View.View_PreExposure;
    out.out_var_SV_Target1 = _509;
    out.out_var_SV_Target2 = _513;
    out.out_var_SV_Target3 = _525;
