...
fragment MainPS_out Main_00003857_48a92774(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _237)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _287 = _281 + _265.y;
        _288 = _281 + _265.x;
        _289 = (_230 * _259.w) + _259.xyz;
        _290 = normalize((_217 * _262.w) + ((_262.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _286 = _235;
    ...
    {
        float3 _321 = fast::clamp((_209 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _525 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_209, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _525 = _497;
    ...
    float4 _533 = float4(((mix(float3(0.0), _382 + (_384 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_475 * _382) * fast::max(float3(1.0), ((((((_289 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_289 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_289 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _526) * 1.0, 0.0);
    float4 _540;
    ...
    float3 _552 = (_290 * 0.5) + float3(0.5);
    float4 _554 = float4(_552.x, _552.y, _552.z, float4(0.0).w);
    _554.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _237)].y;
    ...
    float4 _570 = float4(_289.x, _289.y, _289.z, float4(0.0).w);
