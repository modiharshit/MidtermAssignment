...
vertex Main_out Main_00006a98_2f90fba3(Main_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_MeshParticleVF& MeshParticleVF [[buffer(2)]], texture2d<float> View_AtmosphereTransmittanceTexture [[texture(0)]], texture2d<float> View_AtmosphereIrradianceTexture [[texture(1)]], texture3d<float> View_AtmosphereInscatterTexture [[texture(2)]], texture3d<float> View_CameraAerialPerspectiveVolume [[texture(3)]], sampler View_AtmosphereTransmittanceTextureSampler [[sampler(0)]], sampler View_AtmosphereIrradianceTextureSampler [[sampler(1)]], sampler View_AtmosphereInscatterTextureSampler [[sampler(2)]], sampler View_CameraAerialPerspectiveVolumeSampler [[sampler(3)]])
{
    Main_out out = {};
    spvUnsafeArray<float2, 1> in_var_ATTRIBUTE4 = {};
    in_var_ATTRIBUTE4[0] = in.in_var_ATTRIBUTE4_0;
    float2 _240 = (float2(float(in.in_var_ATTRIBUTE11.x), float(in.in_var_ATTRIBUTE11.y)) + in_var_ATTRIBUTE4[0]) * MeshParticleVF.MeshParticleVF_SubImageSize.xy;
    float2 _247 = (float2(float(in.in_var_ATTRIBUTE11.z), float(in.in_var_ATTRIBUTE11.w)) + in_var_ATTRIBUTE4[0]) * MeshParticleVF.MeshParticleVF_SubImageSize.xy;
    float4x4 _249 = float4x4(in.in_var_ATTRIBUTE8, in.in_var_ATTRIBUTE9, in.in_var_ATTRIBUTE10, float4(0.0, 0.0, 0.0, 1.0));
    float4x4 _250 = transpose(_249);
    float4 _251 = _250 * in.in_var_ATTRIBUTE0;
    float3 _252 = _251.xyz;
    float3 _253 = _252 + View.View_PreViewTranslation;
    float _255 = _253.x;
    float _256 = _253.y;
    float _257 = _253.z;
    float4 _258 = float4(_255, _256, _257, in.in_var_ATTRIBUTE0.w);
    float4 _259 = float4(_258.x, _258.y, _258.z, _258.w);
    float4 _260 = View.View_TranslatedWorldToClip * _259;
    float2 _261 = float2(in.in_var_ATTRIBUTE12.x);
    float4 _264 = _174;
    _264.w = 0.0;
    float3x3 _266 = float3x3(float3(0.0), float3(0.0), float3(0.0));
    _266[2] = in.in_var_ATTRIBUTE2.xyz;
    float3 _269 = cross(in.in_var_ATTRIBUTE2.xyz, in.in_var_ATTRIBUTE1) * in.in_var_ATTRIBUTE2.w;
    float3x3 _270 = _266;
    _270[1] = _269;
    float3x3 _273 = _270;
    _273[0] = cross(_269, in.in_var_ATTRIBUTE2.xyz) * in.in_var_ATTRIBUTE2.w;
    float3x3 _281 = float3x3(_250[0].xyz, _250[1].xyz, _250[2].xyz) * _273;
    float3 _283 = normalize(_281[0]);
    float3 _294 = _258.xyz;
    ...
    if (View.View_SkyAtmosphereApplyCameraAerialPerspectiveVolume > 0.0)
    {
        ...
        {
            _1059 = ((_260.xy / _260.ww) * float2(0.5, -0.5)) + float2(0.5);
        }
        ...
        float4 _1079 = float4(_1076.x, _1076.y, _1076.z, _172.w);
    ...
    out.out_var_TEXCOORD10_centroid = float4(_283.x, _283.y, _283.z, _264.w);
    out.out_var_TEXCOORD11_centroid = float4(normalize(_281[2]), in.in_var_ATTRIBUTE2.w * Primitive.Primitive_InvNonUniformScaleAndDeterminantSign.w);
    out.out_var_TEXCOORD1 = float4(_240.x, _240.y, float2(0.0).x, float2(0.0).y);
    out.out_var_TEXCOORD2 = float4(_247.x, _247.y, _261.x, _261.y);
