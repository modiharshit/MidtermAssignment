...
fragment MainPS_out Main_00002738_0802df63(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_Material& Material [[buffer(2)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], float4 gl_FragCoord [[position]], bool gl_FrontFacing [[front_facing]])
{
    MainPS_out out = {};
    float4 _167 = float4((((gl_FragCoord.xy - View.View_ViewRectMin.xy) * View.View_ViewSizeAndInvSize.zw) - float2(0.5)) * float2(2.0, -2.0), _110, 1.0) * (1.0 / gl_FragCoord.w);
    ...
    float3 _200 = fast::clamp(in.in_var_COLOR0.xyz, float3(0.0), float3(1.0));
    float _204 = (0.5 * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    float _249;
    float _250;
    float _251;
    float3 _252;
    ...
    {
        float2 _221 = ((_167.xy / float2(_167.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        float4 _225 = OpaqueBasePass_DBufferATexture.sample(OpaqueBasePass_DBufferATextureSampler, _221, level(0.0));
        float4 _228 = OpaqueBasePass_DBufferBTexture.sample(OpaqueBasePass_DBufferATextureSampler, _221, level(0.0));
        float4 _231 = OpaqueBasePass_DBufferCTexture.sample(OpaqueBasePass_DBufferATextureSampler, _221, level(0.0));
        _249 = (_204 * _231.w) + _231.z;
        _250 = _231.y;
        _251 = _231.x;
        _252 = (_200 * _225.w) + _225.xyz;
        _253 = normalize((_191 * _228.w) + ((_228.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _249 = _204;
    ...
    {
        float3 _270 = fast::clamp((_177 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float3 _340 = fast::max(mix(in.in_var_COLOR0.xyz, Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _366;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        float3 _365;
        if (any(abs(_177 - Primitive.Primitive_ObjectWorldPositionAndRadius.xyz) > (Primitive.Primitive_ObjectBounds + float3(1.0))))
        {
