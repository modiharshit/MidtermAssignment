...
vertex Main_out Main_000017ef_f2ef5110(Main_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_LandscapeFixedGrid& LandscapeFixedGrid [[buffer(2)]], constant type_LandscapeParameters& LandscapeParameters [[buffer(3)]], texture2d<float> LandscapeParameters_HeightmapTexture [[texture(0)]], sampler LandscapeParameters_HeightmapTextureSampler [[sampler(0)]])
{
    Main_out out = {};
    ...
    float4 _179 = float4(_178.x, _178.y, _178.z, _178.w);
    float4 _180 = View.View_TranslatedWorldToClip * _179;
    float2 _181 = _146.xy;
    float2 _194 = ((_181 * LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.zw) + (in.in_var_ATTRIBUTE0.zw * LandscapeParameters.LandscapeParameters_SubsectionOffsetParams.zz);
    float2 _198 = (((_181 * LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.zw) + _106) + _112;
    out.out_var_TEXCOORD0 = (_181 + LandscapeParameters.LandscapeParameters_SubsectionSizeVertsLayerUVPan.zw) + _148;
    out.out_var_TEXCOORD1 = float4(_194.x, _194.y, _198.x, _198.y);
