...
vertex MainVS_out Main_00002a66_7085ca00(MainVS_in in [[stage_in]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_TranslucentDepthPass& TranslucentDepthPass [[buffer(3)]], constant type_SpriteVF& SpriteVF [[buffer(4)]], constant type_Globals& _Globals [[buffer(5)]], texture_buffer<float> CutoutGeometry [[texture(0)]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]])
{
    MainVS_out out = {};
    ...
    float4 _396 = float4(_372[0].x, _372[0].y, _372[0].z, float4(0.0).w);
    _396.w = _322;
    spvUnsafeArray<float4, 1> _406 = { float4(_336.x, _336.y, float4(0.0).z, float4(0.0).w) };
    out.out_var_TEXCOORD10 = _396;
    out.out_var_TEXCOORD11 = float4(_372[2], float(int(sign(determinant(_372)))));
    out.out_var_TEXCOORD1 = in.in_var_ATTRIBUTE5;
    out_var_TEXCOORD3 = _406;
    out.out_var_TEXCOORD6 = _391.z;
