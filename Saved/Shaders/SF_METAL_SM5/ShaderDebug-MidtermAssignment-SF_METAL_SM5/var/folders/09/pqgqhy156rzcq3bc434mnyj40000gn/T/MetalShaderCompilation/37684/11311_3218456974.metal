...
vertex MainVS_out Main_00002c2f_bfd5c18e(MainVS_in in [[stage_in]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_TranslucentDepthPass& TranslucentDepthPass [[buffer(3)]], constant type_SpriteVF& SpriteVF [[buffer(4)]], constant type_Globals& _Globals [[buffer(5)]], texture_buffer<float> CutoutGeometry [[texture(0)]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]])
{
    MainVS_out out = {};
    ...
    float4 _407 = float4(_382[0].x, _382[0].y, _382[0].z, float4(0.0).w);
    _407.w = _325;
    spvUnsafeArray<float4, 1> _417 = { float4(_343.x, _343.y, float4(0.0).z, float4(0.0).w) };
    out.out_var_TEXCOORD10 = _407;
