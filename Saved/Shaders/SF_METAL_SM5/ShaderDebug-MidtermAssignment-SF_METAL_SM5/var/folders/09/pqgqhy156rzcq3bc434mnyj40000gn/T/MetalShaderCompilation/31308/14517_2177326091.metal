...
fragment MainPS_out Main_000038b5_81c75c0b(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], texture2d<float> Material_Texture2D_4 [[texture(8)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], sampler Material_Texture2D_4Sampler [[sampler(6)]], float4 gl_FragCoord [[position]])
{
    ...
    float2 _230 = ((normalize(-_213) * _193).xy * ((0.00999999977648258209228515625 * _224.w) + (-0.004999999888241291046142578125))) + _218;
    ...
    uint _430 = in.in_var_LIGHTMAP_ID * 15u;
    uint _431 = _430 + 4u;
    uint _436 = _430 + 6u;
    float3 _441 = _422.xyz;
    float3 _469 = ((((_441 * _441) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _431)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _436)].xyz) * ((exp2(((_422.w + ((_424.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _431)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _436)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_424 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _430 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _430 + 7u)], float4(_367.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _519 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_214, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _519 = _491;
    ...
    float4 _527 = float4(((mix(float3(0.0), _397 + (_399 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_469 * _397) * fast::max(_408, ((((((_366 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _308) + ((_366 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _308) + ((_366 * 2.755199909210205078125) + float3(0.69029998779296875))) * _308))) + _520) * 1.0, 0.0);
    float4 _534;
    ...
    float3 _546 = (_367 * 0.5) + float3(0.5);
    float4 _548 = float4(_546.x, _546.y, _546.z, float4(0.0).w);
    _548.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _315)].y;
    ...
    _551.z = _363;
    float4 _552 = _551;
    _552.w = 0.75686275959014892578125;
    float4 _564 = float4(_366.x, _366.y, _366.z, float4(0.0).w);
    _564.w = ((log2(((dot(_469, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_408, ((((((_401 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _308) + ((_401 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _308) + ((_401 * 2.755199909210205078125) + float3(0.69029998779296875))) * _308).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_538.xyx * _538.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
