...
fragment MainPS_out Main_00003bc4_58660f05(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _253 = _246.y;
    float3 _263 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_253)), float3(0.0), float3(1.0));
    float _264 = fast::clamp(mix(0.0, 1.0, _253), 0.0, 1.0);
    float _265 = fast::clamp(_246.x, 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _272)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _324 = (_264 * _310) + _300.x;
        _325 = (_263 * _294.w) + _294.xyz;
    ...
    {
        float4 _436 = LightmapResourceCluster_SkyOcclusionTexture.sample(LightmapResourceCluster_LightMapSampler, in.in_var_TEXCOORD4.xy);
        float _440 = _436.w;
        float _441 = _440 * _440;
        float3 _443 = normalize(((_436.xyz * 2.0) - float3(1.0)).xyz);
        float _444 = 1.0 - _441;
        float _446 = 1.0 - (_444 * _444);
        _452 = mix(fast::clamp(dot(_443, _326), 0.0, 1.0), 1.0, _446);
        _453 = _441;
        _454 = mix(_443, _326, float3(_446));
    }
    else
    {
        _452 = 1.0;
    ...
    float4 _458 = float4(_454, 1.0);
    float3 _462 = _144;
    _462.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 0u)], _458);
    float3 _466 = _462;
    _466.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 1u)], _458);
    float3 _470 = _466;
    _470.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 2u)], _458);
    float4 _473 = _458.xyzz * _458.yzzx;
    float3 _477 = _144;
    _477.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 3u)], _473);
    float3 _481 = _477;
    _481.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 4u)], _473);
    float3 _485 = _481;
    _485.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 5u)], _473);
    float3 _500 = (((((_400 * _400) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _390)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _395)].xyz) * ((exp2(((_381.w + ((_383.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _390)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _395)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_383 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _389 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _389 + 7u)], float4(_326.yzx, 1.0))))) * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_470 + _485) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_454.x * _454.x) - (_454.y * _454.y)))) * View.View_SkyLightColor.xyz) * (_453 * _452));
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _550 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_210, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _550 = _522;
    ...
    float4 _558 = float4(((mix(float3(0.0), _356 + (_358 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_500 * _356) * fast::max(_367, ((((((_325 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _265) + ((_325 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _265) + ((_325 * 2.755199909210205078125) + float3(0.69029998779296875))) * _265))) + _551) * 1.0, 0.0);
    float4 _565;
    ...
    float3 _577 = (_326 * 0.5) + float3(0.5);
    float4 _579 = float4(_577.x, _577.y, _577.z, float4(0.0).w);
    _579.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _272)].y;
    ...
    float4 _595 = float4(_325.x, _325.y, _325.z, float4(0.0).w);
    _595.w = ((log2(((dot(_500, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_367, ((((((_360 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _265) + ((_360 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _265) + ((_360 * 2.755199909210205078125) + float3(0.69029998779296875))) * _265).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_569.xyx * _569.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
