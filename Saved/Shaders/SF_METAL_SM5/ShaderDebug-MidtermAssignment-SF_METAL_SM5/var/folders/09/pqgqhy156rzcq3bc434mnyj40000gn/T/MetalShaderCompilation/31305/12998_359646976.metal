...
fragment MainPS_out Main_000032c6_156fc700(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    ...
    float _431 = (exp2((_412 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_399 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _401 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _401 + 7u)], float4(_343.yzx, 1.0)));
    float3 _441 = float4(_411 * (_431 / _412), _431).xyz * View.View_IndirectLightingColorScale;
    float3 _463 = fast::max(mix((Material.Material_VectorExpressions[2].xyz * _237) * Material.Material_ScalarExpressions[0].x, Material.Material_VectorExpressions[3].xyz, float3(Material.Material_ScalarExpressions[0].y)), float3(0.0));
    float3 _492;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _491 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_204, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _491 = _463;
    ...
    float4 _499 = float4(((mix(float3(0.0), _373 + (_375 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_441 * _373) * fast::max(float3(1.0), ((((((_342 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_342 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_342 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _492) * 1.0, 0.0);
    float4 _506;
    ...
    float3 _518 = (_343 * 0.5) + float3(0.5);
    float4 _520 = float4(_518.x, _518.y, _518.z, float4(0.0).w);
    _520.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _289)].y;
    ...
    float4 _536 = float4(_342.x, _342.y, _342.z, float4(0.0).w);
    _536.w = ((log2(((dot(_441, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_377 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_377 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_377 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_510.xyx * _510.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
