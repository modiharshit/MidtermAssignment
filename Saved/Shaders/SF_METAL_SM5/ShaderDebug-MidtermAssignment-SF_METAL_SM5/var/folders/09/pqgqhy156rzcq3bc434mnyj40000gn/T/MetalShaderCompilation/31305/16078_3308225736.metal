...
fragment MainPS_out Main_00003ece_c52f84c8(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture0 [[texture(5)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture1 [[texture(6)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture2 [[texture(7)]], texture2d<float> Material_Texture2D_0 [[texture(8)]], texture2d<float> Material_Texture2D_1 [[texture(9)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler IndirectLightingCache_IndirectLightingCacheTextureSampler0 [[sampler(2)]], sampler Material_Texture2D_0Sampler [[sampler(3)]], sampler Material_Texture2D_1Sampler [[sampler(4)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float4 _252 = Material_Texture2D_1.sample(Material_Texture2D_1Sampler, float2(in_var_TEXCOORD0[0].x, in_var_TEXCOORD0[0].y), bias(View.View_MaterialTextureMipBias));
    float _256 = _252.z;
    float _279 = _252.y;
    float3 _300 = fast::clamp(mix(mix(Material.Material_VectorExpressions[6].xyz, Material.Material_VectorExpressions[7].xyz, float3(_256)), Material.Material_VectorExpressions[9].xyz, float3(_279)) * _252.x, float3(0.0), float3(1.0));
    float _301 = fast::clamp(mix(mix(Material.Material_ScalarExpressions[0].z, Material.Material_ScalarExpressions[0].w, _256), Material.Material_ScalarExpressions[1].x, _279), 0.0, 1.0);
    float _306 = (fast::clamp(mix(mix(Material.Material_ScalarExpressions[1].y, Material.Material_ScalarExpressions[1].z, _256), Material.Material_ScalarExpressions[1].w, _279), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    ...
    {
        float2 _326 = ((_213.xy / float2(_213.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        _362 = normalize((_248 * _333.w) + ((_333.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _358 = _306;
    ...
    {
        float3 _393 = fast::clamp((_223 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    if (_372)
    {
        ...
        float4 _513 = _159;
        _513.y = (-0.48860299587249755859375) * _362.y;
        float4 _516 = _513;
        _516.z = 0.48860299587249755859375 * _362.z;
        float4 _519 = _516;
        _519.w = (-0.48860299587249755859375) * _362.x;
        float4 _520 = _519;
    ...
    float3 _557 = fast::max(mix((Material.Material_VectorExpressions[2].xyz * _256) * Material.Material_ScalarExpressions[0].x, Material.Material_VectorExpressions[3].xyz, float3(Material.Material_ScalarExpressions[0].y)), float3(0.0));
    float3 _586;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        float3 _585;
        if (any(abs(_223 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _307 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _307 + 19u)].xyz + float3(1.0))))
        {
            _585 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_223, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _585 = _557;
    ...
    float4 _593 = float4(((mix(float3(0.0), _454 + (_456 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_535 * _454) * fast::max(float3(1.0), ((((((_361 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_361 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_361 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _586) * 1.0, 0.0);
    float4 _600;
    ...
    float3 _612 = (_362 * 0.5) + float3(0.5);
    float4 _614 = float4(_612.x, _612.y, _612.z, float4(0.0).w);
    _614.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _308)].y;
    ...
    float4 _630 = float4(_361.x, _361.y, _361.z, float4(0.0).w);
    _630.w = ((log2(((dot(_535, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_458 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_458 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_458 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_604.xyx * _604.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
