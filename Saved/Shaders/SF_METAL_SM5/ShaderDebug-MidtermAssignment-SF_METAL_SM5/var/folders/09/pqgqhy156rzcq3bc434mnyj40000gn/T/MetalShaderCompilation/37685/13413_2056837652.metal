...
fragment MainPS_out Main_00003465_7a98da14(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture2d<float> Material_Texture2D_0 [[texture(5)]], texture2d<float> Material_Texture2D_1 [[texture(6)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _248 = _241.y;
    float3 _258 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_248)), float3(0.0), float3(1.0));
    float _259 = fast::clamp(mix(0.0, 1.0, _248), 0.0, 1.0);
    float _260 = fast::clamp(_241.x, 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _267)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _319 = (_259 * _305) + _295.x;
        _320 = (_258 * _289.w) + _289.xyz;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _495 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_205, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _495 = _467;
    ...
    float4 _503 = float4(((mix(float3(0.0), _399 + (select(_391, float3(0.0), bool3(_394)) * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((((fast::max(float3(0.0), (_417 + _432) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, 6u)].xyz * ((_321.x * _321.x) - (_321.y * _321.y)))) * View.View_SkyLightColor.xyz) * 1.0) * _399) * fast::max(float3(_260), ((((((_320 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _260) + ((_320 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _260) + ((_320 * 2.755199909210205078125) + float3(0.69029998779296875))) * _260))) + _496) * 1.0, 0.0);
    float4 _510;
    ...
    float3 _522 = (_321 * 0.5) + float3(0.5);
    float4 _524 = float4(_522.x, _522.y, _522.z, float4(0.0).w);
    _524.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _267)].y;
    ...
    _527.z = _317;
    float4 _528 = _527;
    _528.w = 0.50588238239288330078125;
    float4 _531 = float4(_320.x, _320.y, _320.z, float4(0.0).w);
    _531.w = (fract(dot(_514.xyx * _514.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125;
