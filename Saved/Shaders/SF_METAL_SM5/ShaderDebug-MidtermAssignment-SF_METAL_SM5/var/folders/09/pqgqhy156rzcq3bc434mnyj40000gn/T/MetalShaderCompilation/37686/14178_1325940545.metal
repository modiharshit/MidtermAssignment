...
fragment MainPS_out Main_00003762_4f083f41(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]], bool gl_FrontFacing [[front_facing]])
{
    MainPS_out out = {};
    ...
    float3 _245 = normalize(float3x3(in.in_var_TEXCOORD10_centroid.xyz, cross(in.in_var_TEXCOORD11_centroid.xyz, in.in_var_TEXCOORD10_centroid.xyz) * in.in_var_TEXCOORD11_centroid.w, in.in_var_TEXCOORD11_centroid.xyz) * normalize((float4(_229, sqrt(fast::clamp(1.0 - dot(_229, _229), 0.0, 1.0)), 1.0).xyz * View.View_NormalOverrideParameter.w) + View.View_NormalOverrideParameter.xyz)) * ((View.View_CullingSign * View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _212 + 4u)].w) * float(gl_FrontFacing ? 1 : (-1)));
    ...
    if ((_256.w - 0.33329999446868896484375) < 0.0)
    {
        discard_fragment();
    }
    float3 _263 = fast::clamp(_256.xyz, float3(0.0), float3(1.0));
    float _267 = (0.5 * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    uint _268 = _212 + 20u;
    float _316;
    float _317;
    float _318;
    ...
    {
        float2 _286 = ((_201.xy / float2(_201.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        float _306 = _296.w;
        _316 = (_267 * _306) + _296.z;
        _317 = (0.5 * _306) + _296.y;
        _318 = _296.x;
        _319 = (_263 * _290.w) + _290.xyz;
        _320 = normalize((_245 * _293.w) + ((_293.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _316 = _267;
    ...
    float3 _335 = ((_319 - (_319 * _318)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _342 = (mix(float3(0.07999999821186065673828125 * _317), _319, float3(_318)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    bool _345 = View.View_RenderingReflectionCaptureMask != 0.0;
    float3 _350;
    if (_345)
    {
        _350 = _335 + (_342 * 0.449999988079071044921875);
    }
    else
    {
        _350 = _335;
    }
    float3 _352 = select(_342, float3(0.0), bool3(_345));
    float3 _354 = float3(dot(_352, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)));
    ...
    float _408 = (exp2((_389 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_376 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _378 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _378 + 7u)], float4(_320.yzx, 1.0)));
    float4 _419 = float4(_320, 1.0);
    float3 _423 = _142;
    ...
    float3 _460 = (float4(_388 * (_408 / _389), _408).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_431 + _446) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_320.x * _320.x) - (_320.y * _320.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    {
        float3 _510;
        if (any(abs(_211 - View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _212 + 5u)].xyz) > (View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _212 + 19u)].xyz + float3(1.0))))
        {
    ...
    float4 _518 = float4(((mix(float3(0.0), _350 + (_352 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_460 * _350) * fast::max(float3(1.0), ((((((_319 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_319 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_319 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _511) * 1.0, 0.0);
    float4 _525;
    ...
    float3 _537 = (_320 * 0.5) + float3(0.5);
    float4 _539 = float4(_537.x, _537.y, _537.z, float4(0.0).w);
    _539.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _268)].y;
    ...
    float4 _555 = float4(_319.x, _319.y, _319.z, float4(0.0).w);
    _555.w = ((log2(((dot(_460, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_354 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_354 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_354 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_529.xyx * _529.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
