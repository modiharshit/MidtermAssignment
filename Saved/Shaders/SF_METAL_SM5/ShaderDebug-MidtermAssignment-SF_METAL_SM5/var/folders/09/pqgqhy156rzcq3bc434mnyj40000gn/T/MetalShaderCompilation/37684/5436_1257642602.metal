...
fragment MainPS_out Main_0000153c_4af61a6a(MainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_Primitive& Primitive [[buffer(1)]], constant type_TranslucentBasePass& TranslucentBasePass [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<float> TranslucentBasePass_Shared_Fog_IntegratedLightScattering [[texture(0)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], float4 gl_FragCoord [[position]])
{
    ...
    float _111 = in_var_TEXCOORD3[0].x - 0.5;
    float _114 = in_var_TEXCOORD3[0].y - 0.5;
    float _117 = 1.0 - ((_111 * _111) + (_114 * _114));
    float _125 = fast::clamp(fast::min(fast::max((_117 <= 0.0) ? 0.0 : pow(_117, 30.0), 0.0), 1.0) * in.in_var_TEXCOORD2.w, 0.0, 1.0);
    ...
    float3 _171 = fast::max(mix(in.in_var_TEXCOORD2.xyz, Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float _198;
    float3 _199;
