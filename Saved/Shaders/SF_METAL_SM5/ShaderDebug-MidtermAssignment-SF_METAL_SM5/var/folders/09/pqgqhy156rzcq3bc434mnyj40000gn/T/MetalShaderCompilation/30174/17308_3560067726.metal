...
fragment MainPS_out Main_0000439c_d432528e(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_PrecomputedLightingBuffer& PrecomputedLightingBuffer [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> LightmapResourceCluster_SkyOcclusionTexture [[texture(4)]], texture2d<float> LightmapResourceCluster_StaticShadowTexture [[texture(5)]], texture2d<float> Material_Texture2D_0 [[texture(6)]], texture2d<float> Material_Texture2D_1 [[texture(7)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    {
        float _331 = fract(cos(dot(floor(gl_FragCoord.xy), float2(347.834503173828125, 3343.28369140625))) * 1000.0);
        if ((float((in.in_var_COLOR1.w < 0.0) ? ((in.in_var_COLOR1.w + 1.0) > _331) : (in.in_var_COLOR1.w < _331)) - 0.001000000047497451305389404296875) < 0.0)
        {
            discard_fragment();
        }
    }
    float3 _342 = fast::clamp(float3(mix(_269, 1.0 - _269, mix(_303, 1.0, _302)) * (mix(0.2949999868869781494140625, 0.660000026226043701171875, mix(_309 + mix(_303, 0.0, _302), 0.5, 0.5)) * 0.5)), float3(0.0), float3(1.0));
    float _347 = (fast::clamp(fast::min(fast::max(mix(0.0, 0.5, _309) + mix(mix(0.699999988079071044921875, 1.0, _303), 1.0, _302), 0.0), 1.0), 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    float _394;
    float _395;
    float _396;
    float3 _397;
    ...
    if ((Primitive.Primitive_DecalReceiverMask > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        float _384 = _374.w;
        _394 = (_347 * _384) + _374.z;
        _395 = (0.5 * _384) + _374.y;
        _396 = _374.x;
        _397 = (_342 * _368.w) + _368.xyz;
        _398 = normalize((_255 * _371.w) + ((_371.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _394 = _347;
    ...
    float4 _412 = fast::clamp((_405 * PrecomputedLightingBuffer.PrecomputedLightingBuffer_InvUniformPenumbraSizes) + ((PrecomputedLightingBuffer.PrecomputedLightingBuffer_InvUniformPenumbraSizes * (-0.5)) + float4(0.5)), float4(0.0), float4(1.0));
    float3 _429 = ((_397 - (_397 * _396)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _436 = (mix(float3(0.07999999821186065673828125 * _395), _397, float3(_396)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    float3 _483 = _467.xyz;
    float _533;
    float _534;
    float3 _535;
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        float4 _517 = LightmapResourceCluster_SkyOcclusionTexture.sample(LightmapResourceCluster_LightMapSampler, in.in_var_TEXCOORD4.xy);
        float _521 = _517.w;
        float _522 = _521 * _521;
        float3 _524 = normalize(((_517.xyz * 2.0) - float3(1.0)).xyz);
        float _525 = 1.0 - _522;
        float _527 = 1.0 - (_525 * _525);
        _533 = mix(fast::clamp(dot(_524, _398), 0.0, 1.0), 1.0, _527);
        _534 = _522;
        _535 = mix(_524, _398, float3(_527));
    }
    else
    {
        _533 = 1.0;
    ...
    float3 _543 = _161;
    _543.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 0u)], _539);
    float3 _547 = _543;
    _547.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 1u)], _539);
    float3 _551 = _547;
    _551.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 2u)], _539);
    float4 _554 = _539.xyzz * _539.yzzx;
    float3 _558 = _161;
    _558.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 3u)], _554);
    float3 _562 = _558;
    _562.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 4u)], _554);
    float3 _566 = _562;
    _566.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 5u)], _554);
    float3 _581 = (((((_483 * _483) * PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapScale[0].xyz) + PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapAdd[0].xyz) * ((exp2(((_467.w + ((_469.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapScale[0].w) + PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapAdd[0].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_469 * PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapScale[1]) + PrecomputedLightingBuffer.PrecomputedLightingBuffer_LightMapAdd[1], float4(_398.yzx, 1.0))))) * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_551 + _566) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 6u)].xyz * ((_535.x * _535.x) - (_535.y * _535.y)))) * View.View_SkyLightColor.xyz) * (_534 * _533));
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _628 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_227, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _628 = _603;
    ...
    float4 _636 = float4(((mix(float3(0.0), _444 + (_446 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_581 * _444) * fast::max(float3(1.0), ((((((_397 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_397 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_397 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _629) * 1.0, 0.0);
    float4 _643;
    ...
    float3 _655 = (_398 * 0.5) + float3(0.5);
    float4 _657 = float4(_655.x, _655.y, _655.z, float4(0.0).w);
    _657.w = Primitive.Primitive_PerObjectGBufferData;
    ...
    float4 _673 = float4(_397.x, _397.y, _397.z, float4(0.0).w);
    _673.w = ((log2(((dot(_581, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_448 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_448 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_448 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_647.xyx * _647.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _643 * View.View_PreExposure;
    out.out_var_SV_Target1 = _657;
    out.out_var_SV_Target2 = _661;
    out.out_var_SV_Target3 = _673;
    out.out_var_SV_Target4 = float4(0.0);
    out.out_var_SV_Target5 = (PrecomputedLightingBuffer.PrecomputedLightingBuffer_StaticShadowMapMasks * _412) * _412;
    return out;
}
