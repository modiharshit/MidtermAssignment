...
vertex Main_out Main_00002b5a_73a9f570(Main_in in [[stage_in]], constant type_View& View [[buffer(2)]], constant type_Primitive& Primitive [[buffer(3)]], constant type_LandscapeContinuousLODParameters& LandscapeContinuousLODParameters [[buffer(4)]], constant type_LandscapeParameters& LandscapeParameters [[buffer(5)]], texture_buffer<float> LandscapeContinuousLODParameters_SectionLOD [[texture(0)]], texture_buffer<float> LandscapeContinuousLODParameters_SectionLODBias [[texture(1)]], texture2d<float> LandscapeParameters_HeightmapTexture [[texture(2)]], sampler LandscapeParameters_HeightmapTextureSampler [[sampler(0)]])
{
    ...
    float2 _328 = _295.xy;
    float4 _333 = float4(_327.x, _327.y, _327.z, _327.w);
    float4 _334 = View.View_TranslatedWorldToClip * _333;
    float2 _343 = ((_328 * LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_WeightmapUVScaleBias.zw) + (in.in_var_ATTRIBUTE0.zw * LandscapeParameters.LandscapeParameters_SubsectionOffsetParams.zz);
    float2 _347 = (((_328 * LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.xy) + LandscapeParameters.LandscapeParameters_HeightmapUVScaleBias.zw) + _254) + _259;
    out.out_var_TEXCOORD0 = (_328 + LandscapeParameters.LandscapeParameters_SubsectionSizeVertsLayerUVPan.zw) + _297;
    out.out_var_TEXCOORD1 = float4(_343.x, _343.y, _347.x, _347.y);
    out.out_var_TEXCOORD2 = float4(0.0);
    out.gl_Position = _334;
    return out;
}
