...
fragment MainPS_out Main_00002d0f_3e04a2c0(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _364 = ((((_336 * _336) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _326)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331)].xyz) * ((exp2(((_317.w + ((_319.w * 0.0039215688593685626983642578125) - 0.00196078442968428134918212890625)) * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _326)].w) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _331)].w) - 0.0185813605785369873046875) * fast::max(0.0, dot((_319 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _325 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _325 + 7u)], float4(_263.yzx, 1.0))))) * View.View_IndirectLightingColorScale;
    float3 _386 = fast::max(mix(float3(0.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0));
    float3 _415;
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _414 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_190, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _414 = _386;
    ...
    float4 _422 = float4(((mix(float3(0.0), _293 + (_295 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_364 * _293) * fast::max(float3(1.0), ((((((_262 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_262 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_262 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _415) * 1.0, 0.0);
    float4 _429;
    ...
    float3 _441 = (_263 * 0.5) + float3(0.5);
    float4 _443 = float4(_441.x, _441.y, _441.z, float4(0.0).w);
    _443.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _211)].y;
    ...
    _446.z = _259;
    float4 _447 = _446;
    _447.w = 0.75686275959014892578125;
    float4 _459 = float4(_262.x, _262.y, _262.z, float4(0.0).w);
    _459.w = ((log2(((dot(_364, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_297 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_297 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_297 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_433.xyx * _433.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
    out.out_var_SV_Target0 = _429 * View.View_PreExposure;
    out.out_var_SV_Target1 = _443;
    out.out_var_SV_Target2 = _447;
    out.out_var_SV_Target3 = _459;
