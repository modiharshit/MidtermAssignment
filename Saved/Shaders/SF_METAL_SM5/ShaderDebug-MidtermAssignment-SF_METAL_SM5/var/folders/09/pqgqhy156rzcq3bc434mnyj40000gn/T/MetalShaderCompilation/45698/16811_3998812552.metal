...
fragment MainPS_out Main_000041ab_ee590988(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture0 [[texture(5)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture1 [[texture(6)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture2 [[texture(7)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler IndirectLightingCache_IndirectLightingCacheTextureSampler0 [[sampler(2)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _235 = fast::clamp(Material.Material_ScalarExpressions[0].y, 0.0, 1.0);
    float _240 = (fast::clamp(Material.Material_ScalarExpressions[0].z, 0.0, 1.0) * View.View_RoughnessOverrideParameter.y) + View.View_RoughnessOverrideParameter.x;
    float _287;
    float _288;
    float _289;
    float3 _290;
    ...
    {
        float2 _257 = ((_201.xy / float2(_201.w)) * View.View_ScreenPositionScaleBias.xy) + View.View_ScreenPositionScaleBias.wz;
        ...
        float _277 = _267.w;
        _287 = (_240 * _277) + _267.z;
        _288 = (_235 * _277) + _267.y;
        _289 = _267.x;
        _290 = (_234 * _261.w) + _261.xyz;
        _291 = normalize((_219 * _264.w) + ((_264.xyz * 2.0) - float3(1.00392162799835205078125)));
    }
    else
    {
        _287 = _240;
    ...
    float _305;
    if ((Primitive.Primitive_UseSingleSampleShadowFromStationaryLights > 0.0) && _299)
    {
    ...
    {
        float3 _320 = fast::clamp((_211 * View.View_VolumetricLightmapWorldToUVScale) + View.View_VolumetricLightmapWorldToUVAdd, float3(0.0), float3(0.9900000095367431640625)) * float3(View.View_VolumetricLightmapIndirectionTextureSize);
    ...
    float3 _366 = ((_290 - (_290 * _289)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _373 = (mix(float3(0.07999999821186065673828125 * _288), _290, float3(_289)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    if (_299)
    {
        float3 _411 = fast::clamp((_211 * IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveScale) + IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveAdd, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMinUV, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMaxUV);
        ...
        float4 _440 = _147;
        _440.y = (-0.48860299587249755859375) * _291.y;
        float4 _443 = _440;
        _443.z = 0.48860299587249755859375 * _291.z;
        float4 _446 = _443;
        _446.w = (-0.48860299587249755859375) * _291.x;
        float4 _447 = _446;
    ...
    if (View.View_SkyLightApplyPrecomputedBentNormalShadowingFlag != 0.0)
    {
        ...
        _485 = mix(fast::clamp(dot(_475, _291), 0.0, 1.0), 1.0, _479);
        _486 = _476;
        _487 = mix(_475, _291, float3(_479));
    }
    else
    {
        _485 = 1.0;
    ...
    float3 _495 = _144;
    _495.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 0u)], _491);
    float3 _499 = _495;
    _499.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 1u)], _491);
    float3 _503 = _499;
    _503.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 2u)], _491);
    float4 _506 = _491.xyzz * _491.yzzx;
    float3 _510 = _144;
    _510.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 3u)], _506);
    float3 _514 = _510;
    _514.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 4u)], _506);
    float3 _518 = _514;
    _518.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 5u)], _506);
    float3 _533 = (_459 * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_503 + _518) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, 6u)].xyz * ((_487.x * _487.x) - (_487.y * _487.y)))) * View.View_SkyLightColor.xyz) * (_486 * _485));
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _580 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_211, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _580 = _555;
    ...
    float4 _588 = float4(((mix(float3(0.0), _381 + (_383 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_533 * _381) * fast::max(float3(1.0), ((((((_290 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_290 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_290 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _581) * 1.0, 0.0);
    float4 _595;
    ...
    float3 _607 = (_291 * 0.5) + float3(0.5);
    float4 _609 = float4(_607.x, _607.y, _607.z, float4(0.0).w);
    _609.w = Primitive.Primitive_PerObjectGBufferData;
    ...
    float4 _625 = float4(_290.x, _290.y, _290.z, float4(0.0).w);
    _625.w = ((log2(((dot(_533, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_385 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_385 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_385 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_599.xyx * _599.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
