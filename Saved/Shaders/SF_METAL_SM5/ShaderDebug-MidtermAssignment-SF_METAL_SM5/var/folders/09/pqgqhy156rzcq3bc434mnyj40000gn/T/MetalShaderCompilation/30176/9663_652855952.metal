...
vertex MainForGS_out Main_000025bf_26e9ca90(MainForGS_in in [[stage_in]], constant type_View& View [[buffer(4)]], constant type_Primitive& Primitive [[buffer(5)]], constant type_ShadowDepthPass& ShadowDepthPass [[buffer(6)]], constant type_LocalVF& LocalVF [[buffer(7)]], constant type_Globals& _Globals [[buffer(8)]], texture_buffer<float> InstanceVF_VertexFetch_InstanceOriginBuffer [[texture(0)]], texture_buffer<float> InstanceVF_VertexFetch_InstanceTransformBuffer [[texture(1)]], texture_buffer<float> LocalVF_VertexFetch_TexCoordBuffer [[texture(2)]], texture_buffer<float> LocalVF_VertexFetch_PackedTangentsBuffer [[texture(3)]], uint gl_InstanceIndex [[instance_id]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]], uint gl_BaseInstance [[base_instance]])
{
    ...
    float3x3 _123 = _122;
    _123[2] = _116;
    float3x3 _137 = float3x3(Primitive.Primitive_LocalToWorld[0].xyz, Primitive.Primitive_LocalToWorld[1].xyz, Primitive.Primitive_LocalToWorld[2].xyz) * float3x3(_102.xyz, _104.xyz, _106.xyz);
    float3x3 _140 = _137;
    ...
    float4 _277 = LocalVF_VertexFetch_TexCoordBuffer.read(uint(((_271 * _96) + min(0u, (_271 - 1u)))));
    float3 _278 = _147[2];
    float4 _279 = float4(_268.x, _268.y, _268.z, _268.w);
    spvUnsafeArray<float4, 1> _290 = { float4(_277.x, _277.y, float4(0.0).z, float4(0.0).w) };
    float4 _295 = ShadowDepthPass.ShadowDepthPass_ShadowViewProjectionMatrices[_Globals.LayerId] * _279;
    out.out_var_TEXCOORD10_centroid = float4(_147[0], 0.0);
    out.out_var_TEXCOORD11_centroid = float4(_278, _115 * Primitive.Primitive_InvNonUniformScaleAndDeterminantSign.w);
    out.out_var_COLOR1 = _235;
    out_var_TEXCOORD0 = _290;
    out.out_var_TEXCOORD6 = _295;
    out.out_var_TEXCOORD8 = _278;
    out.gl_Layer = _Globals.LayerId;
    out.gl_Position = _295;
