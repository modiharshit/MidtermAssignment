...
vertex Main_out Main_0000782a_aadcc057(Main_in in [[stage_in]], constant type_View& View [[buffer(4)]], constant type_Primitive& Primitive [[buffer(5)]], constant type_ShadowDepthPass& ShadowDepthPass [[buffer(6)]], constant type_NiagaraSpriteVFLooseParameters& NiagaraSpriteVFLooseParameters [[buffer(7)]], constant type_NiagaraSpriteVF& NiagaraSpriteVF [[buffer(8)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_CutoutGeometry [[texture(0)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_NiagaraParticleDataFloat [[texture(1)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_NiagaraParticleDataHalf [[texture(2)]], texture_buffer<int> NiagaraSpriteVFLooseParameters_SortedIndices [[texture(3)]], uint gl_VertexIndex [[vertex_id]], uint gl_InstanceIndex [[instance_id]], uint gl_BaseVertex [[base_vertex]], uint gl_BaseInstance [[base_instance]])
{
    ...
    float3 _692 = _688 / float3(fast::max(length(_688), 9.9999999747524270787835121154785e-07));
    float _729;
    switch (0u)
    {
        default:
    ...
    float _829 = (_268 * NiagaraSpriteVF.NiagaraSpriteVF_RotationScale) + NiagaraSpriteVF.NiagaraSpriteVF_RotationBias;
    float _830 = sin(_829);
    float _831 = cos(_829);
    float3 _836 = (_822 * _830) + (_823 * _831);
    float3 _837 = (_822 * _831) - (_823 * _830);
    ...
    float3x3 _895 = _894;
    _895[1] = _837;
    bool _898 = NiagaraSpriteVF.NiagaraSpriteVF_NormalsType < 0.5;
    float3x3 _929;
    if (_898)
    {
        float3x3 _928 = _895;
        _928[2] = normalize(cross(_836, _837));
        _929 = _928;
    }
    else
    {
        ...
        {
            float3x3 _924 = _895;
            _924[2] = normalize(_230 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz);
            _925 = _924;
        }
        else
        {
            float3x3 _918 = _895;
            _918[2] = normalize(_230 - (NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz + (NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz * dot(NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz, _230 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz))));
            _925 = _918;
        }
        _929 = _925;
    }
    float4 _934 = float4((_230 + ((_692 * _729) + (float2x3(_836, _837) * (abs(float4(_319, _268, _446).xy) * (_880 - _687))))) + View.View_PreViewTranslation, 1.0);
    float3x3 _965;
    if (_898)
    {
        float3x3 _964 = _895;
        _964[2] = normalize(cross(_836, _837));
        _965 = _964;
    }
    else
    {
        ...
        {
            float3x3 _960 = _895;
            _960[2] = normalize(_230 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz);
            _961 = _960;
        }
        else
        {
            float3x3 _954 = _895;
            _954[2] = normalize(_230 - (NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz + (NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz * dot(NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz, _230 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz))));
            _961 = _954;
        }
        _965 = _961;
    }
    float4 _972 = ShadowDepthPass.ShadowDepthPass_ProjectionMatrix * float4(_934.x, _934.y, _934.z, _934.w);
    float4 _983;
    if ((ShadowDepthPass.ShadowDepthPass_bClampToNearPlane > 0.0) && (_972.z < 0.0))
    {
        float4 _981 = _972;
        _981.z = 9.9999999747524270787835121154785e-07;
        float4 _982 = _981;
        _982.w = 1.0;
        _983 = _982;
    }
    ...
    float _989 = abs(dot(float3(ShadowDepthPass.ShadowDepthPass_ViewMatrix[0].z, ShadowDepthPass.ShadowDepthPass_ViewMatrix[1].z, ShadowDepthPass.ShadowDepthPass_ViewMatrix[2].z), _965[2]));
    float4 _1010 = float4(_929[0].x, _929[0].y, _929[0].z, float4(0.0).w);
    _1010.w = (NiagaraSpriteVF.NiagaraSpriteVF_SubImageBlendMode == 1) ? fract(_446) : 0.0;
    out.out_var_TEXCOORD10 = _1010;
    out.out_var_TEXCOORD11 = float4(_929[2], float(int(sign(determinant(_929)))));
    out.out_var_TEXCOORD6 = _983.z;
    out.out_var_TEXCOORD8 = (ShadowDepthPass.ShadowDepthPass_ShadowParams.y * fast::clamp((abs(_989) > 0.0) ? (sqrt(fast::clamp(1.0 - (_989 * _989), 0.0, 1.0)) / _989) : ShadowDepthPass.ShadowDepthPass_ShadowParams.z, 0.0, ShadowDepthPass.ShadowDepthPass_ShadowParams.z)) + ShadowDepthPass.ShadowDepthPass_ShadowParams.x;
    out.out_var_TEXCOORD7 = _934.xyz;
    out.gl_Position = _983;
    return out;
}
