...
vertex MainVS_out Main_00002de3_027f74c7(MainVS_in in [[stage_in]], constant type_View& View [[buffer(1)]], constant type_Primitive& Primitive [[buffer(2)]], constant type_TranslucentDepthPass& TranslucentDepthPass [[buffer(3)]], constant type_SpriteVF& SpriteVF [[buffer(4)]], constant type_Globals& _Globals [[buffer(5)]], texture_buffer<float> CutoutGeometry [[texture(0)]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]])
{
    MainVS_out out = {};
    ...
    float _424 = _420.z * TranslucentDepthPass.TranslucentDepthPass_InvMaxSubjectDepth;
    float4 _427 = _420;
    _427.z = _424 * _420.w;
    float4 _432 = float4(_390[0].x, _390[0].y, _390[0].z, float4(0.0).w);
    _432.w = _333;
    spvUnsafeArray<float4, 1> _442 = { float4(_351.x, _351.y, float4(0.0).z, float4(0.0).w) };
