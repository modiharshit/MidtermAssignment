...
fragment MainPS_out Main_00003fb8_0fb71ed5(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], texture2d<float> Material_Texture2D_4 [[texture(8)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], sampler Material_Texture2D_4Sampler [[sampler(6)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _484 = (exp2((_465 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_452 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _454 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _454 + 7u)], float4(_395.yzx, 1.0)));
    float4 _495 = float4(_395, 1.0);
    float3 _499 = _157;
    ...
    float3 _536 = (float4(_464 * (_484 / _465), _484).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_507 + _522) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_395.x * _395.x) - (_395.y * _395.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _586 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_222, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _586 = _558;
    ...
    float4 _594 = float4(((mix(float3(0.0), _425 + (_427 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_536 * _425) * fast::max(_436, ((((((_394 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _336) + ((_394 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _336) + ((_394 * 2.755199909210205078125) + float3(0.69029998779296875))) * _336))) + _587) * 1.0, 0.0);
    float4 _601;
    ...
    float3 _613 = (_395 * 0.5) + float3(0.5);
    float4 _615 = float4(_613.x, _613.y, _613.z, float4(0.0).w);
    _615.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _343)].y;
    ...
    _618.z = _391;
    float4 _619 = _618;
    _619.w = 0.75686275959014892578125;
    float4 _631 = float4(_394.x, _394.y, _394.z, float4(0.0).w);
    _631.w = ((log2(((dot(_536, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_436, ((((((_429 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _336) + ((_429 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _336) + ((_429 * 2.755199909210205078125) + float3(0.69029998779296875))) * _336).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_605.xyx * _605.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
