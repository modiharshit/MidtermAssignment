...
fragment MainPS_out Main_0000313b_2787aa21(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], constant type_View& View [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float3 _380 = (_366.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _370 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _370 + 6u)].xyz;
    float _381 = dot(_380, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _400 = (exp2((_381 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_368 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _370 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _370 + 7u)], float4(_312.yzx, 1.0)));
    float3 _410 = float4(_380 * (_400 / _381), _400).xyz * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _460 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_201, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _460 = _432;
    ...
    float4 _468 = float4(((mix(float3(0.0), _342 + (_344 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_410 * _342) * fast::max(float3(1.0), ((((((_311 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_311 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_311 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _461) * 1.0, 0.0);
    float4 _475;
    ...
    float3 _487 = (_312 * 0.5) + float3(0.5);
    float4 _489 = float4(_487.x, _487.y, _487.z, float4(0.0).w);
    _489.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _258)].y;
    ...
    _492.z = _308;
    float4 _493 = _492;
    _493.w = 0.75686275959014892578125;
    float4 _505 = float4(_311.x, _311.y, _311.z, float4(0.0).w);
    _505.w = ((log2(((dot(_410, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(float3(1.0), ((((((_346 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_346 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_346 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_479.xyx * _479.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
