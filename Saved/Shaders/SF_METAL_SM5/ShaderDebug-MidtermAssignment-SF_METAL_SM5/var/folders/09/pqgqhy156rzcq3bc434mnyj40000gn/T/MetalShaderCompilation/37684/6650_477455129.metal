...
vertex MainVS_out Main_000019fa_1c756319(MainVS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(6)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_TranslucentDepthPass& TranslucentDepthPass [[buffer(4)]], constant type_LocalVF& LocalVF [[buffer(5)]], texture_buffer<float> LocalVF_VertexFetch_TexCoordBuffer [[texture(0)]], texture_buffer<float> LocalVF_VertexFetch_PackedTangentsBuffer [[texture(1)]], uint gl_VertexIndex [[vertex_id]], uint gl_BaseVertex [[base_vertex]])
{
    MainVS_out out = {};
    ...
    float4 _167 = LocalVF_VertexFetch_TexCoordBuffer.read(uint((uint(LocalVF.LocalVF_VertexFetch_Parameters.y) * _77)));
    spvUnsafeArray<float4, 1> _179 = { float4(_167.x, _167.y, float4(0.0).z, float4(0.0).w) };
    ...
    out_var_TEXCOORD0 = _179;
    out.out_var_PRIMITIVE_ID = in.in_var_ATTRIBUTE13;
    out.out_var_TEXCOORD6 = _160.z;
