...
vertex Main_out Main_0000726d_2a687697(Main_in in [[stage_in]], constant type_View& View [[buffer(4)]], constant type_Primitive& Primitive [[buffer(5)]], constant type_ShadowDepthPass& ShadowDepthPass [[buffer(6)]], constant type_NiagaraSpriteVFLooseParameters& NiagaraSpriteVFLooseParameters [[buffer(7)]], constant type_NiagaraSpriteVF& NiagaraSpriteVF [[buffer(8)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_CutoutGeometry [[texture(0)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_NiagaraParticleDataFloat [[texture(1)]], texture_buffer<float> NiagaraSpriteVFLooseParameters_NiagaraParticleDataHalf [[texture(2)]], texture_buffer<int> NiagaraSpriteVFLooseParameters_SortedIndices [[texture(3)]], uint gl_VertexIndex [[vertex_id]], uint gl_InstanceIndex [[instance_id]], uint gl_BaseVertex [[base_vertex]], uint gl_BaseInstance [[base_instance]])
{
    ...
    float3 _686 = _682 / float3(fast::max(length(_682), 9.9999999747524270787835121154785e-07));
    float _723;
    switch (0u)
    {
        default:
    ...
    float _823 = (_262 * NiagaraSpriteVF.NiagaraSpriteVF_RotationScale) + NiagaraSpriteVF.NiagaraSpriteVF_RotationBias;
    float _824 = sin(_823);
    float _825 = cos(_823);
    float3 _830 = (_816 * _824) + (_817 * _825);
    float3 _831 = (_816 * _825) - (_817 * _824);
    ...
    float4 _887 = float4((_224 + ((_686 * _723) + (float2x3(_830, _831) * (abs(float4(_313, _262, _440).xy) * (_874 - _681))))) + View.View_PreViewTranslation, 1.0);
    float3x3 _921;
    if (NiagaraSpriteVF.NiagaraSpriteVF_NormalsType < 0.5)
    {
        float3x3 _920 = _86;
        _920[2] = normalize(cross(_830, _831));
        _921 = _920;
    }
    else
    {
        ...
        {
            float3x3 _916 = _86;
            _916[2] = normalize(_224 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz);
            _917 = _916;
        }
        else
        {
            float3x3 _910 = _86;
            _910[2] = normalize(_224 - (NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz + (NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz * dot(NiagaraSpriteVF.NiagaraSpriteVF_NormalsCylinderUnitDirection.xyz, _224 - NiagaraSpriteVF.NiagaraSpriteVF_NormalsSphereCenter.xyz))));
            _917 = _910;
        }
        _921 = _917;
    }
    float4 _928 = ShadowDepthPass.ShadowDepthPass_ProjectionMatrix * float4(_887.x, _887.y, _887.z, _887.w);
    float4 _939;
    if ((ShadowDepthPass.ShadowDepthPass_bClampToNearPlane > 0.0) && (_928.z < 0.0))
    {
        float4 _937 = _928;
        _937.z = 9.9999999747524270787835121154785e-07;
        float4 _938 = _937;
        _938.w = 1.0;
        _939 = _938;
    }
    ...
    float _945 = abs(dot(float3(ShadowDepthPass.ShadowDepthPass_ViewMatrix[0].z, ShadowDepthPass.ShadowDepthPass_ViewMatrix[1].z, ShadowDepthPass.ShadowDepthPass_ViewMatrix[2].z), _921[2]));
    out.out_var_TEXCOORD6 = _939.z;
    out.out_var_TEXCOORD8 = (ShadowDepthPass.ShadowDepthPass_ShadowParams.y * fast::clamp((abs(_945) > 0.0) ? (sqrt(fast::clamp(1.0 - (_945 * _945), 0.0, 1.0)) / _945) : ShadowDepthPass.ShadowDepthPass_ShadowParams.z, 0.0, ShadowDepthPass.ShadowDepthPass_ShadowParams.z)) + ShadowDepthPass.ShadowDepthPass_ShadowParams.x;
    out.gl_Position = _939;
    return out;
}
