...
fragment MainPS_out Main_00003c77_5bafc1d7(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(5)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], const device type_StructuredBuffer_v4float& View_LightmapSceneData [[buffer(1)]], const device type_StructuredBuffer_v4float& View_SkyIrradianceEnvironmentMap [[buffer(2)]], constant type_View& View [[buffer(3)]], constant type_Material& Material [[buffer(4)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(0)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(2)]], texture2d<float> LightmapResourceCluster_LightMapTexture [[texture(3)]], texture2d<float> Material_Texture2D_0 [[texture(4)]], texture2d<float> Material_Texture2D_1 [[texture(5)]], texture2d<float> Material_Texture2D_2 [[texture(6)]], texture2d<float> Material_Texture2D_3 [[texture(7)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(0)]], sampler LightmapResourceCluster_LightMapSampler [[sampler(1)]], sampler Material_Texture2D_0Sampler [[sampler(2)]], sampler Material_Texture2D_1Sampler [[sampler(3)]], sampler Material_Texture2D_2Sampler [[sampler(4)]], sampler Material_Texture2D_3Sampler [[sampler(5)]], float4 gl_FragCoord [[position]])
{
    ...
    float3 _377 = ((_361 - (_361 * _360)) * View.View_DiffuseOverrideParameter.w) + View.View_DiffuseOverrideParameter.xyz;
    float3 _384 = (mix(float3(0.07999999821186065673828125 * _359), _361, float3(_360)) * View.View_SpecularOverrideParameter.w) + View.View_SpecularOverrideParameter.xyz;
    ...
    float3 _431 = (_417.xyz * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _421 + 4u)].xyz) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _421 + 6u)].xyz;
    float _432 = dot(_431, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375));
    float _451 = (exp2((_432 * 16.0) - 8.0) - 0.00390625) * fast::max(0.0, dot((_419 * View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _421 + 5u)]) + View_LightmapSceneData._m0[spvStorageBufferCoords(1, spvBufferSizeConstants, float4, _421 + 7u)], float4(_362.yzx, 1.0)));
    float4 _462 = float4(_362, 1.0);
    float3 _466 = _150;
    _466.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 0u)], _462);
    float3 _470 = _466;
    _470.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 1u)], _462);
    float3 _474 = _470;
    _474.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 2u)], _462);
    float4 _477 = _462.xyzz * _462.yzzx;
    float3 _481 = _150;
    _481.x = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 3u)], _477);
    float3 _485 = _481;
    _485.y = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 4u)], _477);
    float3 _489 = _485;
    _489.z = dot(View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 5u)], _477);
    float3 _503 = (float4(_431 * (_451 / _432), _451).xyz * View.View_IndirectLightingColorScale) + ((fast::max(float3(0.0), (_474 + _489) + (View_SkyIrradianceEnvironmentMap._m0[spvStorageBufferCoords(2, spvBufferSizeConstants, float4, 6u)].xyz * ((_362.x * _362.x) - (_362.y * _362.y)))) * View.View_SkyLightColor.xyz) * 1.0);
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _553 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_215, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _553 = _525;
    ...
    float4 _561 = float4(((mix(float3(0.0), _392 + (_394 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_503 * _392) * fast::max(_403, ((((((_361 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _303) + ((_361 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _303) + ((_361 * 2.755199909210205078125) + float3(0.69029998779296875))) * _303))) + _554) * 1.0, 0.0);
    float4 _568;
    ...
    float3 _580 = (_362 * 0.5) + float3(0.5);
    float4 _582 = float4(_580.x, _580.y, _580.z, float4(0.0).w);
    _582.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _310)].y;
    ...
    float4 _598 = float4(_361.x, _361.y, _361.z, float4(0.0).w);
    _598.w = ((log2(((dot(_503, float3(0.300000011920928955078125, 0.589999973773956298828125, 0.10999999940395355224609375)) * fast::max(_403, ((((((_396 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * _303) + ((_396 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * _303) + ((_396 * 2.755199909210205078125) + float3(0.69029998779296875))) * _303).y) * View.View_PreExposure) + 0.00390625) * 0.0625) + 0.5) + ((fract(dot(_572.xyx * _572.xyy, float3(20.390625, 60.703125, 2.4281208515167236328125))) - 0.5) * 0.0039215688593685626983642578125);
