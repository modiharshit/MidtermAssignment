...
fragment MainPS_out Main_00003d74_f88c7c2a(MainPS_in in [[stage_in]], constant uint* spvBufferSizeConstants [[buffer(4)]], const device type_StructuredBuffer_v4float& View_PrimitiveSceneData [[buffer(0)]], constant type_View& View [[buffer(1)]], constant type_IndirectLightingCache& IndirectLightingCache [[buffer(2)]], constant type_Material& Material [[buffer(3)]], texture3d<uint> View_VolumetricLightmapIndirectionTexture [[texture(0)]], texture3d<float> View_DirectionalLightShadowingBrickTexture [[texture(1)]], texture2d<float> OpaqueBasePass_DBufferATexture [[texture(2)]], texture2d<float> OpaqueBasePass_DBufferBTexture [[texture(3)]], texture2d<float> OpaqueBasePass_DBufferCTexture [[texture(4)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture0 [[texture(5)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture1 [[texture(6)]], texture3d<float> IndirectLightingCache_IndirectLightingCacheTexture2 [[texture(7)]], texture2d<float> Material_Texture2D_0 [[texture(8)]], texture2d<float> Material_Texture2D_1 [[texture(9)]], sampler View_SharedBilinearClampedSampler [[sampler(0)]], sampler OpaqueBasePass_DBufferATextureSampler [[sampler(1)]], sampler IndirectLightingCache_IndirectLightingCacheTextureSampler0 [[sampler(2)]], sampler Material_Texture2D_0Sampler [[sampler(3)]], sampler Material_Texture2D_1Sampler [[sampler(4)]], float4 gl_FragCoord [[position]])
{
    MainPS_out out = {};
    ...
    float _265 = _258.y;
    float3 _276 = fast::clamp(mix(Material.Material_VectorExpressions[4].xyz, Material.Material_VectorExpressions[5].xyz, float3(_265)) * _258.x, float3(0.0), float3(1.0));
    float _277 = fast::clamp(mix(0.0, 1.0, _265), 0.0, 1.0);
    ...
    if ((View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _284)].x > 0.0) && (View.View_ShowDecalsMask > 0.0))
    {
        ...
        _336 = (_277 * _322) + _312.x;
        _337 = (_276 * _306.w) + _306.xyz;
    ...
    if (_348)
    {
        float3 _460 = fast::clamp((_222 * IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveScale) + IndirectLightingCache.IndirectLightingCache_IndirectLightingCachePrimitiveAdd, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMinUV, IndirectLightingCache.IndirectLightingCache_IndirectLightingCacheMaxUV);
        float4 _464 = IndirectLightingCache_IndirectLightingCacheTexture0.sample(IndirectLightingCache_IndirectLightingCacheTextureSampler0, _460);
        float4 _468 = IndirectLightingCache_IndirectLightingCacheTexture1.sample(View_SharedBilinearClampedSampler, _460);
        float4 _471 = IndirectLightingCache_IndirectLightingCacheTexture2.sample(View_SharedBilinearClampedSampler, _460);
        float4 _489 = _158;
        _489.y = (-0.48860299587249755859375) * _338.y;
        float4 _492 = _489;
        _492.z = 0.48860299587249755859375 * _338.z;
        float4 _495 = _492;
        _495.w = (-0.48860299587249755859375) * _338.x;
        float4 _496 = _495;
        ...
        _501.x = dot(float4(_464.x, _468.x, _471.x, _464.w), _499);
        float3 _503 = _501;
        _503.y = dot(float4(_464.y, _468.y, _471.y, _468.w), _499);
        float3 _505 = _503;
        _505.z = dot(float4(_464.z, _468.z, _471.zw), _499);
        _508 = fast::max(float3(0.0), _505) * float3(0.3183098733425140380859375);
    }
    else
    {
        _508 = float3(0.0);
    }
    float3 _511 = _508 * View.View_IndirectLightingColorScale;
    ...
    if (View.View_OutOfBoundsMask > 0.0)
    {
        ...
        {
            _561 = mix(float3(1.0, 1.0, 0.0), float3(0.0, 1.0, 1.0), select(float3(0.0), float3(1.0), float3(fract(dot(_222, float3(0.57700002193450927734375)) * 0.00200000009499490261077880859375)) > float3(0.5)));
        }
        else
        {
            _561 = _533;
    ...
    float4 _569 = float4(((mix(float3(0.0), _430 + (_432 * 0.449999988079071044921875), float3(View.View_UnlitViewmodeMask)) + ((_511 * _430) * fast::max(float3(1.0), ((((((_337 * 2.040400028228759765625) - float3(0.3323999941349029541015625)) * 1.0) + ((_337 * (-4.79510021209716796875)) + float3(0.6417000293731689453125))) * 1.0) + ((_337 * 2.755199909210205078125) + float3(0.69029998779296875))) * 1.0))) + _562) * 1.0, 0.0);
    float4 _576;
    ...
    float3 _588 = (_338 * 0.5) + float3(0.5);
    float4 _590 = float4(_588.x, _588.y, _588.z, float4(0.0).w);
    _590.w = View_PrimitiveSceneData._m0[spvStorageBufferCoords(0, spvBufferSizeConstants, float4, _284)].y;
