...
fragment MainOpacityPS_out Main_00000aa4_df2e9f2e(MainOpacityPS_in in [[stage_in]], constant type_Globals& _Globals [[buffer(0)]])
{
    MainOpacityPS_out out = {};
    spvUnsafeArray<float4, 1> in_var_TEXCOORD3 = {};
    in_var_TEXCOORD3[0] = in.in_var_TEXCOORD3_0;
    float2 _52 = float2(in_var_TEXCOORD3[0].x, in_var_TEXCOORD3[0].y) - float2(0.5);
    float _58 = fast::min(fast::max(1.0 - (sqrt(dot(_52, _52)) * 2.0), 0.0), 1.0);
    float _64 = fast::clamp(in.in_var_TEXCOORD2.w * ((_58 <= 0.0) ? 0.0 : pow(_58, 4.0)), 0.0, 1.0);
