...
fragment InjectMainPS_out Main_000032ff_be831ad8(InjectMainPS_in in [[stage_in]], constant type_View& View [[buffer(0)]], constant type_DeferredLightUniforms& DeferredLightUniforms [[buffer(1)]], constant type_Material& Material [[buffer(2)]], constant type_Globals& _Globals [[buffer(3)]], texture2d<float> View_TransmittanceLutTexture [[texture(0)]], texture2d<float> ShadowDepthTexture [[texture(1)]], texture2d<float> StaticShadowDepthTexture [[texture(2)]], texture2d<float> VolumetricCloudShadowmapTexture [[texture(3)]], sampler View_TransmittanceLutTextureSampler [[sampler(0)]], sampler ShadowDepthTextureSampler [[sampler(1)]], sampler StaticShadowDepthTextureSampler [[sampler(2)]], sampler VolumetricCloudShadowmapTextureSampler [[sampler(3)]], uint gl_Layer [[render_target_array_index]])
{
    ...
    float _315 = fast::min(_256, mix(_289, 1.0, _302 * _302)) * dot(fast::max(mix(float3(1.0), Material.Material_VectorExpressions[1].xyz, float3(Material.Material_ScalarExpressions[0].x)), float3(0.0)), float3(0.33329999446868896484375));
    float _354;
